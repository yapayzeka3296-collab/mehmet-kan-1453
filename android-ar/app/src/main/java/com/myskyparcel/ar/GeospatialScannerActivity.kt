...
class GeospatialScannerActivity : ComponentActivity() {
    ...
    override fun onResume() {
        ...
        val request = LocationRequest.Builder(Priority.PRIORITY_HIGH_ACCURACY, 1000L)
            .setMinUpdateIntervalMillis(500L).setWaitForAccurateLocation(false).build()
        locationClient.requestLocationUpdates(request, locationCallback, mainLooper)
    }

    override fun onPause() { locationClient.removeLocationUpdates(locationCallback); super.onPause() }
...
private fun ScannerRoot(
    ...
    LaunchedEffect(location) {
        location?.let { loc ->
            val name = runCatching {
                Geocoder(activity, Locale("tr", "TR")).getFromLocation(loc.latitude, loc.longitude, 1)?.firstOrNull()?.adminArea
            }.getOrNull()
    ...
    fun checkArCore() {
        ...
        ArCoreApk.getInstance().checkAvailabilityAsync(activity) { availability ->
            if (!availability.isSupported) { mode = "unsupported"; return@checkAvailabilityAsync }
            mode = try {
    ...
    DisposableEffect(cameraGranted) { checkArCore(); onDispose { } }
    ...
    Box(Modifier.fillMaxSize()) {
        ARSceneView(
            ...
            sessionConfiguration = { session, config ->
                ...
                config.depthMode = if (session.isDepthModeSupported(Config.DepthMode.AUTOMATIC)) Config.DepthMode.AUTOMATIC else Config.DepthMode.DISABLED
            },
            onSessionUpdated = { session, _ ->
                val earth = session.earth
                earthTracking = earth?.trackingState == TrackingState.TRACKING
                if (!earthTracking || earth == null) {
                    ...
                    if (loaded && available.size >= 3 && !anchorsCreated && pose.horizontalAccuracy <= 30.0 && pose.verticalAccuracy <= 30.0 && pose.orientationYawAccuracy <= 30.0) {
                        val testDistances = doubleArrayOf(1000.0, 1500.0, 2000.0)
                        val testBearings = doubleArrayOf(0.0, 120.0, 240.0)
                        available.take(3).forEachIndexed { index, parcel ->
                            val point = GeoMath.destination(pose.latitude, pose.longitude, pose.altitude, testBearings[index], 20.0, testDistances[index])
                            runCatching { earth.createAnchor(point.latitude, point.longitude, point.altitude, 0f, 0f, 0f, 1f) }.getOrNull()?.let { anchor ->
                                placed += PlacedParcel(parcel.copy(isTest = true, latitude = point.latitude, longitude = point.longitude), anchor)
                            }
                        }
                        available.drop(3).take(6).forEach { parcel ->
                            runCatching { earth.createAnchor(parcel.latitude, parcel.longitude, pose.altitude + 150.0, 0f, 0f, 0f, 1f) }.getOrNull()?.let { anchor -> placed += PlacedParcel(parcel, anchor) }
                        }
                        parcels.filter { it.status == "sold" }.take(3).forEach { parcel ->
                            runCatching { earth.createAnchor(parcel.latitude, parcel.longitude, pose.altitude + 150.0, 0f, 0f, 0f, 1f) }.getOrNull()?.let { anchor -> placed += PlacedParcel(parcel, anchor) }
                        }
                        anchorsCreated = true
                    }
            ...
            onGestureListener = rememberOnGestureListener(onSingleTapConfirmed = { _, node -> node?.name?.takeIf { it.isNotBlank() }?.let(selectParcel) })
        ) {
            placed.forEach { item ->
                key(item.parcel.id) {
                        ...
                        BillboardNode {
                            ...
                            TextNode(
                                ...
                                backgroundColor = when {
                                    item.parcel.isTest -> 0xDDFF8A00.toInt()
                                    sold -> 0xDD6B1F2B.toInt()
                                    else -> 0xDD06111F.toInt()
                                },
                                widthMeters = 1.8f,
                                heightMeters = 0.44f,
                                apply = {
        ...
        Column(Modifier.align(Alignment.TopCenter).padding(14.dp).background(Color(0xDD06111F), MaterialTheme.shapes.large).padding(horizontal = 16.dp, vertical = 12.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(3.dp)) {
...
private object GeoMath {
    ...
    fun destination(latitude: Double, longitude: Double, altitude: Double, bearingDeg: Double, elevationDeg: Double, slantDistanceMeters: Double): Destination {
        ...
        val lat = latitude + Math.toDegrees(north / EARTH_RADIUS)
        val lon = longitude + Math.toDegrees(east / (EARTH_RADIUS * cos(Math.toRadians(latitude))))
        return Destination(lat, lon, altitude + slantDistanceMeters * sin(elevation))
    }
}

@Composable private fun PermissionScreen(cameraGranted: Boolean, requestPermissions: () -> Unit) {
