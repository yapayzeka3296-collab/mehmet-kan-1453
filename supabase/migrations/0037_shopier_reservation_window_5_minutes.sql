-- Shorten Shopier checkout parcel reservations to five minutes.
-- Also return a distinct error when another user currently holds the parcel.
...
begin
  ...
  if v_existing.id is not null then
    ...
    return jsonb_build_object('ok',true,'intent_id',v_existing.id,'order_ids',v_existing.order_ids,'parcel_ids',v_existing.parcel_ids,'certificate_parcel_id',v_certificate,'amount',v_existing.amount,'currency',v_existing.currency,'expires_at',v_existing.expires_at,'reused',true);
  end if;
  ...
  for v_id2 in select unnest(v_requested) loop
    ...
    if v_parcel.status='reserved' and v_parcel.reserved_by is not null and v_parcel.reserved_by<>v_user_id then
      raise exception 'parcel_reserved_by_other_user';
    end if;
  ...
  return jsonb_build_object('ok',true,'intent_id',v_id,'order_ids',v_orders,'parcel_ids',v_parcels,'certificate_parcel_id',v_certificate,'amount',v_total,'currency','TRY','expires_at',now()+interval '5 minutes');
end;
$$;
