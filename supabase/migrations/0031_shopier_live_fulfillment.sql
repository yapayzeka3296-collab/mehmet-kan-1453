...
begin
  ...
  if coalesce(array_length(v_intent.order_ids,1),0)>0 then
    update public.orders set status='paid',provider='shopier',provider_reference=p_shopier_order_id||':'||id::text,updated_at=now() where id=any(v_intent.order_ids) and user_id=v_intent.user_id;
  elsif v_intent.order_id is not null then
    update public.orders set status='paid',provider='shopier',provider_reference=p_shopier_order_id||':'||v_intent.order_id::text,updated_at=now() where id=v_intent.order_id and user_id=v_intent.user_id;
  end if;
  if coalesce(array_length(v_intent.order_ids,1),0)>0 then
