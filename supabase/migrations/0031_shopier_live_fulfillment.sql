...
begin
  ...
  where p.id=any(v_intent.parcel_ids) and p.status='reserved'
    and p.reserved_by=v_intent.user_id and p.reserved_until is not null and p.reserved_until>now();
  ...
  if coalesce(array_length(v_intent.order_ids,1),0)>0 then
    update public.orders set status='paid',provider='shopier',provider_reference=p_shopier_order_id||':'||id::text,updated_at=now()
    where id=any(v_intent.order_ids) and user_id=v_intent.user_id;
  elsif v_intent.order_id is not null then
    update public.orders set status='paid',provider='shopier',provider_reference=p_shopier_order_id||':'||v_intent.order_id::text,updated_at=now()
  ...
  if coalesce(array_length(v_intent.order_ids,1),0)>0 then
    foreach v_order_id in array v_intent.order_ids loop
      ...
      if v_payment_id is null then
        ...
        values(v_order_id,v_intent.user_id,(select amount from public.orders where id=v_order_id),'TRY','succeeded','shopier',coalesce(p_shopier_payment_id,p_shopier_order_id)||':'||v_order_id::text,now()) returning id into v_payment_id;
      else
        update public.payments set status='succeeded',provider='shopier',provider_payment_id=coalesce(p_shopier_payment_id,p_shopier_order_id)||':'||v_order_id::text,paid_at=coalesce(paid_at,now()),updated_at=now() where id=v_payment_id;
      end if;
    end loop;
  elsif v_intent.order_id is not null then
    ...
    if v_payment_id is null then
      insert into public.payments(order_id,user_id,amount,currency,status,provider,provider_payment_id,paid_at)
      values(v_intent.order_id,v_intent.user_id,(select amount from public.orders where id=v_intent.order_id),'TRY','succeeded','shopier',coalesce(p_shopier_payment_id,p_shopier_order_id)||':'||v_intent.order_id::text,now()) returning id into v_payment_id;
    else
      update public.payments set status='succeeded',provider='shopier',provider_payment_id=coalesce(p_shopier_payment_id,p_shopier_order_id)||':'||v_intent.order_id::text,paid_at=coalesce(paid_at,now()),updated_at=now() where id=v_payment_id;
    end if;
  end if;
  ...
  if v_intent.certificate_parcel_id is not null then
    ...
    if v_cert_tier is not null then
      ...
      insert into public.certificate_requests(user_id,parcel_id,tier,status,holder_name_snapshot,city_name_snapshot,template_type,template_version,production_status)
      values(v_intent.user_id,v_intent.certificate_parcel_id,v_cert_tier,'requested',v_holder_name,v_city_name,v_cert_tier,v_cert_tier||'-v1','request_received') on conflict do nothing;
    end if;
  end if;
...
exception when unique_violation then
  if exists(select 1 from public.shopier_checkout_intents where id=p_intent_id and status='paid') then return jsonb_build_object('ok',true,'status','already_completed','intent_id',p_intent_id); end if;
  raise;
end;
$$;
