-- Expire stale Shopier intents before a retry so the active-intent unique index
-- cannot block a new checkout after the 15-minute reservation window.
...
begin
  ...
  -- Expire stale active intents first. Otherwise the partial unique index on
  -- active parcel reservations can prevent a new checkout intent after expiry.
  ...
  -- Reuse the same user's active intent on checkout retry.
