-- Release stale Shopier reservations before evaluating a new checkout.
-- Without this cleanup, parcels whose 5-minute reservation expired could remain
-- permanently marked as reserved until another code path happened to touch them.
...
begin
  ...
  where status in ('pending','redirected') and expires_at <= now() and parcel_ids && v_requested;
