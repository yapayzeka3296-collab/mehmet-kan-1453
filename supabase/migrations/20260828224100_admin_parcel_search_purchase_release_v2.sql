...
create function public.admin_search_parcels(p_city_slug text default null,p_query text default null,p_only_sold boolean default false)
...
begin
  ...
  where ((not p_only_sold and p.status='available' and p.owner_id is null) or (p_only_sold and p.status='sold' and p.owner_id is not null))
