-- Admin parcel search: keep the full parcels table as the search scope,
-- while returning at most 50 matching rows for a responsive admin UI.
...
begin
  ...
  return query
    select p.id,p.parcel_number,p.status,p.owner_id,coalesce(pr.full_name,''),p.tier,p.price,c.slug,c.name
