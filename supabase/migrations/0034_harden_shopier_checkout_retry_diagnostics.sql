-- Keep checkout diagnostics schema-compatible with the live checkout handler.
alter table public.shopier_checkout_intents
  add column if not exists shopier_api_status integer,
  add column if not exists shopier_api_error text,
  add column if not exists shopier_api_request_id text;

-- Make the active-intent lookup deterministic for retries of the same parcel set.
create index if not exists shopier_checkout_intents_active_user_idx
  on public.shopier_checkout_intents(user_id, created_at desc)
  where status in ('pending','redirected');

-- Recreate the checkout-intent function with exact parcel-set reuse, a 15-minute
-- reservation, and a certificate parcel that must belong to the selected set.
-- A requested certificate parcel may change while the active reservation is reused.
...
begin
  ...
  if coalesce(array_length(v_requested,1),0)=0 then raise exception 'empty_parcel_selection'; end if;
  if array_length(v_requested,1) > 100 then raise exception 'too_many_parcels'; end if;
  ...
  if v_existing.id is not null then
    ...
    if p_certificate_parcel_id is not null and p_certificate_parcel_id is distinct from v_existing.certificate_parcel_id then
      update public.shopier_checkout_intents
      set certificate_parcel_id = p_certificate_parcel_id, updated_at = now()
    ...
    return jsonb_build_object(
      ...
      'shopier_product_id', v_existing.shopier_product_id,
  ...
  for v_id2 in select unnest(v_requested) loop
    ...
    v_orders := array_append(v_orders,v_order.id); v_parcels := array_append(v_parcels,v_id2); v_total := v_total + v_order.amount;
    if v_first is null then v_first:=v_id2; end if;
  end loop;
  ...
  return jsonb_build_object('ok',true,'intent_id',v_id,'order_ids',v_orders,'parcel_ids',v_parcels,'certificate_parcel_id',v_certificate,'amount',v_total,'currency','TRY','expires_at',now()+interval '15 minutes','reused',false);
end;
$$;
