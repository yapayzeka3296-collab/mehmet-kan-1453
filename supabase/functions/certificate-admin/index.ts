...
const corsHeaders = { "Access-Control-Allow-Origin": "https://myskyparcel.com", "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type", "Access-Control-Allow-Methods": "POST, OPTIONS", "Content-Type": "application/json" };
...
Deno.serve(async (req) => {
  ...
  if (!url || !publishableKey || !serviceKey) return new Response(JSON.stringify({ error: "service_unavailable" }), { status: 503, headers: corsHeaders });
  ...
  let result;
  if (body.action === "approve") result = await adminClient.rpc("approve_certificate_request", { p_request_id: body.request_id });
  else if (body.action === "issue") result = await adminClient.rpc("issue_certificate_request", { p_request_id: body.request_id });
  else if (body.action === "reject") result = await adminClient.rpc("reject_certificate_request", { p_request_id: body.request_id, p_reason: body.reason ?? "" });
  else if (body.action === "revoke") result = await adminClient.rpc("revoke_certificate", { p_request_id: body.request_id, p_reason: body.reason ?? "" });
