...
import { _ as Scene, a as CanvasTexture, c as DirectionalLight, d as MeshBasicMaterial, f as MeshPhongMaterial, g as SRGBColorSpace, h as PointsMaterial, i as BufferGeometry, l as MathUtils, m as Points, n as AmbientLight, o as Clock, p as PerspectiveCamera, r as BufferAttribute, s as Color, t as WebGLRenderer, u as Mesh, v as SphereGeometry } from "../_libs/three.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/MySkyParcelEarthGlobeSafe-DoVgRBnv.mjs
...
function makeTexture(renderer, clouds = false) {
	...
	if (clouds) for (const [x, y, rx, ry] of [
		...
		for (const p of shapes) {
			ctx.beginPath();
			p.forEach(([x, y], i) => i ? ctx.lineTo(x * 1024, y * 512) : ctx.moveTo(x * 1024, y * 512));
			ctx.closePath();
			ctx.fill();
			ctx.stroke();
		}
...
function MySkyParcelEarthGlobeSafe({ className = "" }) {
	...
	(0, import_react.useEffect)(() => {
		...
		if (!mount) return;
		let renderer, frame = 0, stopped = false, dragging = false, lastX = 0, lastY = 0;
		...
		try {
			...
			scene.add(new AmbientLight(7243949, .5));
			const sun = new DirectionalLight(16777215, 2.7);
			sun.position.set(5, 3, 5);
			scene.add(sun);
			const earthTex = makeTexture(renderer), cloudTex = makeTexture(renderer, true);
			if (!earthTex || !cloudTex) throw new Error("Texture creation failed");
			const r = RADIUS * (window.matchMedia("(max-width:767px)").matches ? .7 : 1);
			const geo = new SphereGeometry(r, 96, 96), mat = new MeshPhongMaterial({
				map: earthTex,
				shininess: 8,
				specular: new Color(1848656)
			}), earth = new Mesh(geo, mat);
			scene.add(earth);
			const cgeo = new SphereGeometry(r * 1.014, 72, 72), cmat = new MeshPhongMaterial({
				color: 16777215,
				alphaMap: cloudTex,
				transparent: true,
				opacity: .42,
				depthWrite: false
			}), clouds = new Mesh(cgeo, cmat);
			scene.add(clouds);
			const ageo = new SphereGeometry(r * 1.085, 72, 72), amat = new MeshBasicMaterial({
				color: 5089023,
				transparent: true,
				opacity: .13,
				side: 1,
				blending: 2,
				depthWrite: false
			}), atmosphere = new Mesh(ageo, amat);
			scene.add(atmosphere);
			const sgeo = new BufferGeometry(), pos = /* @__PURE__ */ new Float32Array(6600);
			for (let i = 0; i < 2200; i++) {
				const rad = 11 + i % 2800 / 100, th = i * 2.399963, z = i * .6180339887 % 2 - 1, xy = Math.sqrt(1 - z * z);
			...
			const smat = new PointsMaterial({
				color: 16777215,
				size: .048
			}), stars = new Points(sgeo, smat);
			scene.add(stars);
			const sync = () => stars.rotation.copy(earth.rotation), setZoom = (z) => {
				zoomRef.current = MathUtils.clamp(z, MIN_ZOOM, MAX_ZOOM);
				camera.position.z = zoomRef.current;
			}, wheel = (e) => {
				e.preventDefault();
				setZoom(camera.position.z + MathUtils.clamp(e.deltaY, -160, 160) * .008);
			}, down = (e) => {
				...
				if (pointers.size === 2 && pinch) {
					const [a, b] = [...pointers.values()], d = Math.max(1, Math.hypot(a.x - b.x, a.y - b.y));
				...
				earth.rotation.x = MathUtils.clamp(earth.rotation.x + dy * .0035, -1.15, 1.15);
