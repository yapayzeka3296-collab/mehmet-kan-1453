...
import { o as useAuth, r as Route$24 } from "./router-BAmmia3p.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/gokyuzu-haritasi-Dzb206o0.js
...
function geometryToRings(geometry) {
	if (!geometry) return [];
	const convertRing = (source) => {
		const result = [];
		for (const point of source) {
			const lng = point[0];
			const lat = point[1];
			if (typeof lng === "number" && Number.isFinite(lng) && typeof lat === "number" && Number.isFinite(lat)) result.push({
				lat,
				lng
			});
		}
...
function fallbackParcelRings(center) {
	const dLat = .0012;
	const dLng = .0015 / Math.max(Math.cos(center.lat * Math.PI / 180), .25);
	return [[[
		{
			lat: center.lat - dLat,
			lng: center.lng - dLng
...
var ease = (t) => t < .5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2;
var lerp = (a, b, t) => a + (b - a) * t;
function animateCamera(map, from, to, duration, maps) {
	let frame = 0;
...
function FocusedGoogleParcelMap({ parcels, selectedId, selectedIds = EMPTY_SELECTED_IDS, multiSelect = false, onSelect, onToggleSelect, onViewportChange, center, focusTarget }) {
	...
	const animation = (0, import_react.useRef)(null);
	const selectedIdRef = (0, import_react.useRef)(selectedId);
	const selectedIdsRef = (0, import_react.useRef)(selectedIds);
	const multiRef = (0, import_react.useRef)(multiSelect);
	const onSelectRef = (0, import_react.useRef)(onSelect);
	const onToggleRef = (0, import_react.useRef)(onToggleSelect);
	const parcelsRef = (0, import_react.useRef)(parcels);
	...
	(0, import_react.useEffect)(() => {
		...
		animation.current = animateCamera(map.current, from, {
			...center,
			zoom: CITY_ZOOM
		}, 1800, maps);
		return () => {
	...
	(0, import_react.useEffect)(() => {
		...
		parcelsRef.current.forEach((parcel) => {
			...
			const geometryParts = ringsByPolygon.length ? ringsByPolygon : fallbackParcelRings({
				lat: parcel.latitude,
			...
			geometryParts.forEach((rings) => {
				...
				const polygon = new maps.Polygon({
					map: map.current,
					paths,
					strokeColor: selected ? "#fff4b0" : color(parcel),
					strokeOpacity: 1,
					strokeWeight: selected ? 3 : parcel.status === "sold" ? 2.5 : 1.2,
					fillColor: color(parcel),
					fillOpacity: selected ? .18 : parcel.status === "sold" ? .12 : parcel.status === "reserved" ? .09 : .035,
					clickable: true,
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			...
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
				className: "max-w-xl text-xs leading-5 text-white/70",
				children: error
			})
		})]
	});
