...
//#region node_modules/.nitro/vite/services/ssr/assets/gokyuzu-CKf7zOlY.mjs
	...
	(0, import_react.useEffect)(() => {
		...
		const loadCityParcels = async (cityIndex, minX, maxX, minY, maxY) => {
			...
			try {
				...
				const result = await supabaseBrowser.from("parcel_map_public").select("id,parcel_number,status,price,tier,city_name,city_code,layer_number,sector_number,grid_x,grid_y").eq("city_name", city.name).eq("city_code", city.code).gte("grid_x", safeMinX).lte("grid_x", safeMaxX).gte("grid_y", safeMinY).lte("grid_y", safeMaxY).order("grid_y", { ascending: true }).order("grid_x", { ascending: true });
		...
		const onSelectPointerUp = (event) => {
			...
			const parcel = raycaster.intersectObjects(realMeshes, false)[0]?.object?.userData?.parcel;
