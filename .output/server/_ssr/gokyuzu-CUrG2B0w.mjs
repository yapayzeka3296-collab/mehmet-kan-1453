...
//#region node_modules/.nitro/vite/services/ssr/assets/gokyuzu-CUrG2B0w.mjs
...
function GokyuzuPage() {
	...
	(0, import_react.useEffect)(() => {
		...
		const applyDrag = (dx, dy) => {
			const nextX = MathUtils.clamp(panX + dx * dragScale, -110, maxPanX);
			const nextZ = MathUtils.clamp(panZ + dy * dragScale, -55, maxPanZ);
			...
			if (nextX === -110 || nextX === maxPanX) velocityX = 0;
			if (nextZ === -55 || nextZ === maxPanZ) velocityZ = 0;
		...
		const animate = () => {
			...
			if (!dragging && pointers.size === 0 && (Math.abs(velocityX) > inertiaStop || Math.abs(velocityZ) > inertiaStop)) {
				const nextX = MathUtils.clamp(panX + velocityX, -110, maxPanX);
				const nextZ = MathUtils.clamp(panZ + velocityZ, -55, maxPanZ);
				if (nextX === -110 || nextX === maxPanX) velocityX = 0;
				if (nextZ === -55 || nextZ === maxPanZ) velocityZ = 0;
				applyPan(nextX, nextZ);
				velocityX *= friction;
