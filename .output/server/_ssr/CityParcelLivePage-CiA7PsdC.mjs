...
import { U as Heart, k as MapPin, m as ShoppingCart, mt as ArrowLeft, n as X } from "../_libs/lucide-react.mjs";
import { a as removeParcelFromCart, i as readParcelCart, n as PARCEL_CART_EVENT, o as writeParcelCart } from "./SiteHeader-DWNb9nW5.mjs";
import { t as ParcelDetailPanel } from "./ParcelDetailPanel-DP6ZXUxv.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/CityParcelLivePage-CiA7PsdC.mjs
...
function spreadSlotIndexes(indexes, count) {
	...
	while (selected.length < count && remaining.size) {
		...
		for (const candidate of remaining) {
			...
			const minDistance = selected.length === 0 ? (x - 11 / 2) ** 2 + (y - 2) ** 2 : Math.min(...selected.map((slot) => {
				const sx = slot % COLS;
...
function buildSlots(rows) {
	...
	for (const tier of TIER_ORDER) {
		...
		soldIndexes.forEach((index, position) => {
			result[index] = soldRows[position] ?? null;
		});
