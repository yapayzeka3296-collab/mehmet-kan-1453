...
//#region node_modules/.nitro/vite/services/ssr/assets/gokyuzu-kiJ1bjY6.mjs
...
function GokyuzuPage() {
	...
	(0, import_react.useEffect)(() => {
		...
		const onDragMove = (event) => {
			...
			lastPointerX = event.clientX;
			lastPointerY = event.clientY;
			const speed = Math.max(.08, controls.target.distanceTo(camera.position) * .006);
		...
		const onWheel = (event) => {
			...
			const delta = MathUtils.clamp(distance * (event.deltaY > 0 ? 1.12 : .89), 18, 150) - distance;
