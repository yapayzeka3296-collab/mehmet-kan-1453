...
import { o as useAuth, r as Route$24 } from "./router-CIry4i44.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/gokyuzu-haritasi-DTuwlKqH.js
var import_react = /* @__PURE__ */ __toESM(require_react());
var import_jsx_runtime = require_jsx_runtime();
var EMPTY_SELECTED_IDS = /* @__PURE__ */ new Set();
var TURKEY_CENTER = {
	lat: 39,
	lng: 35
};
...
function FocusedGoogleParcelMap({ parcels, selectedId, selectedIds = EMPTY_SELECTED_IDS, multiSelect = false, onSelect, onToggleSelect, onViewportChange, center, focusTarget }) {
	...
	const polygons = (0, import_react.useRef)(/* @__PURE__ */ new Map());
	const multiSelectRef = (0, import_react.useRef)(multiSelect);
	const onSelectRef = (0, import_react.useRef)(onSelect);
	const onToggleSelectRef = (0, import_react.useRef)(onToggleSelect);
	...
	(0, import_react.useEffect)(() => {
		...
		const l = maps.event.addListener(map.current, "idle", () => {
		...
		[
			...
			ps.forEach((p, i) => {
				...
				const polygon = new maps.Polygon({
					map: map.current,
					paths,
					strokeColor: selected ? "#fff4b0" : c,
					strokeOpacity: 1,
					strokeWeight: selected ? 2.5 : p.status === "sold" ? 2.5 : 1.1,
					fillColor: c,
					fillOpacity: selected ? .17 : p.status === "sold" ? .08 : .008,
					clickable: true,
					zIndex: selected ? 501 : p.status === "sold" ? 500 : 21
				});
				polygon.addListener("click", () => {
					if (multiSelectRef.current && p.status === "available") onToggleSelectRef.current?.(p.id);
					else onSelectRef.current(p.id);
				});
				polygons.current.set(p.id, {
					polygon,
					parcel: p
				});
			});
	...
	(0, import_react.useEffect)(() => {
		...
		polygons.current.forEach(({ polygon, parcel }) => {
			...
			polygon.setOptions({
				strokeColor: selected ? "#fff4b0" : c,
				strokeWeight: selected ? 2.5 : parcel.status === "sold" ? 2.5 : 1.1,
				fillOpacity: selected ? .17 : parcel.status === "sold" ? .08 : .008,
				zIndex: selected ? 501 : parcel.status === "sold" ? 500 : 21
			});
		});
	}, [
		ready,
...
var DEFAULT_CITY = PILOT_CITIES[0];
var TIER_BY_NUMBER = (number) => number <= 500 ? "digital" : number <= 800 ? "elite" : "premium";
var TIER_PRICE = {
	digital: 199,
	elite: 499,
	premium: 999
...
function Harita() {
	const { city: citySlug, parcels: focusParcelQuery, lat: focusLat, lng: focusLng } = Route$24.useSearch();
	...
	const [error, setError] = (0, import_react.useState)(null);
	const [manualFocusTarget, setManualFocusTarget] = (0, import_react.useState)(null);
	...
	const queryFocusTarget = (0, import_react.useMemo)(() => {
		if (!focusParcelId || !Number.isFinite(Number(focusLat)) || !Number.isFinite(Number(focusLng))) return null;
		return {
			city: selectedCityMeta.center,
			parcel: {
				lat: Number(focusLat),
				lng: Number(focusLng)
			},
			token: `${focusParcelId}:${focusLat}:${focusLng}`
		};
	}, [
		focusParcelId,
		focusLat,
		focusLng,
		selectedCityMeta.center
	]);
	...
	const selectedParcels = (0, import_react.useMemo)(() => parcels.filter((parcel) => selectedIds.has(parcel.id)), [parcels, selectedIds]);
	const selectedTotal = (0, import_react.useMemo)(() => selectedParcels.reduce((sum, parcel) => sum + Number(parcel.tier_price ?? TIER_PRICE[parcel.tier ?? "digital"]), 0), [selectedParcels]);
	const selectedParcelQuery = (0, import_react.useMemo)(() => selectedParcels.map((parcel) => parcel.id).join(","), [selectedParcels]);
	...
	const filteredCities = (0, import_react.useMemo)(() => {
		...
		if (!query) return PILOT_CITIES;
		return PILOT_CITIES.filter((city) => city.name.toLocaleLowerCase("tr-TR").includes(query));
	}, [citySearch]);
	(0, import_react.useEffect)(() => {
		const nextCity = PILOT_CITIES.find((city) => city.slug === citySlug) ?? DEFAULT_CITY;
		setSelectedCity(nextCity.code);
		setSelectedId(null);
	...
	const loadViewportParcels = (0, import_react.useCallback)(async (bounds) => {
		...
		try {
			...
			const normalized = (data ?? []).map((parcel) => {
				...
				const tier = parcel.tier ?? TIER_BY_NUMBER(numericCode);
				return {
					...parcel,
					owner_id: null,
					tier,
			...
			setSelectedIds((current) => new Set([...current].filter((id) => normalized.some((parcel) => parcel.id === id))));
	...
	const handleLocate = (parcel) => {
		setManualFocusTarget({
			...
			token: `manual:${parcel.id}:${Date.now()}`
		});
	};
	const handleReserved = (reservedParcel) => {
		setParcels((current) => current.map((parcel) => parcel.id === reservedParcel.id ? {
			...parcel,
			...reservedParcel,
			status: "reserved"
		} : parcel));
	...
	const removeSelected = (id) => setSelectedIds((current) => {
		const next = new Set(current);
		next.delete(id);
		return next;
	});
	const goToPurchase = () => {
		if (!selectedParcelQuery) return;
		navigate({
			to: "/parsel-satin-al",
			search: { parcels: selectedParcelQuery }
		});
	};
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("main", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("aside", {
						className: "order-2 border-t border-white/10 bg-slate-950/90 p-4 backdrop-blur-md lg:order-1 lg:border-r lg:border-t-0 lg:p-5",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "mb-4 flex min-h-[150px] flex-col justify-center rounded-2xl border border-cyan-300/20 bg-slate-900/80 p-4 text-center shadow-lg",
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ShieldCheck, { className: "mx-auto h-7 w-7 text-cyan-200" }),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
										className: "mt-2 text-[11px] font-bold uppercase tracking-[0.14em] text-white/85",
										children: "GÜVENLİK"
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
										className: "mt-1 text-[10px] leading-4 text-white/55",
										children: "Güvenlik altyapısı yayın öncesi ayrıca doğrulanmalıdır."
									})
								]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "flex items-center gap-2 rounded-xl border border-white/10 bg-slate-900/80 px-3",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Search, { className: "h-4 w-4 shrink-0 text-sky-100/60" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
									value: citySearch,
									onChange: (event) => setCitySearch(event.target.value),
									placeholder: "Şehir ara...",
									className: "min-w-0 flex-1 bg-transparent py-3 text-sm text-white outline-none placeholder:text-white/40",
									"aria-label": "Pilot şehir ara"
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "mt-5",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
										className: "text-[11px] font-semibold uppercase tracking-[0.16em] text-sky-100/55",
										children: "Pilot şehirler"
									}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(MapPin, { className: "h-4 w-4 text-sky-200/55" })]
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									className: "grid gap-1.5",
									children: filteredCities.map((city) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
										type: "button",
										onClick: () => selectCity(city.code),
										className: `flex items-center gap-2 rounded-xl border px-3 py-2.5 text-left text-sm transition ${selectedCity === city.code ? "border-sky-200/40 bg-sky-200/10 text-white" : "border-transparent text-white/65 hover:border-white/10 hover:bg-white/5 hover:text-white"}`,
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(MapPin, { className: "h-4 w-4 shrink-0" }), city.name]
									}, city.code))
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										...
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Layers, { className: "h-4 w-4 text-sky-200/75" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
											className: "text-[11px] font-semibold uppercase tracking-[0.16em] text-sky-100/60",
									...
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("select", {
										...
										className: "mt-2 w-full rounded-xl border border-white/10 bg-slate-900 px-3 py-2.5 text-sm text-white outline-none focus:border-sky-200/35",
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "mt-6 rounded-2xl border border-white/10 bg-slate-900/65 p-4",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
									className: "text-[11px] font-semibold uppercase tracking-[0.16em] text-sky-100/55",
									children: "Parsel türleri"
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [
										/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
											...
											children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
												...
												children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", { className: "h-2.5 w-2.5 rounded-full bg-sky-300" }), "Dijital"]
											}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "199 TL" })]
										}),
										/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
											className: "flex items-center justify-between text-white/70",
											children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
												className: "flex items-center gap-2",
												children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", { className: "h-2.5 w-2.5 rounded-full bg-violet-300" }), "Elit"]
											}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "499 TL" })]
										}),
										/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
											className: "flex items-center justify-between text-white/70",
											children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
												className: "flex items-center gap-2",
												children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", { className: "h-2.5 w-2.5 rounded-full bg-amber-300" }), "Premium"]
											}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "999 TL" })]
										})
									]
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "mt-5 rounded-2xl border border-white/10 bg-slate-900/65 p-4 text-xs text-white/55",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
									className: "font-semibold text-white/75",
									children: "Harita kullanımı"
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
									className: "mt-2 leading-5",
									children: "Haritayı parmağınla sürükle veya yakınlaştır. Renkli alanlar MySkyParcel parsellerini gösterir."
								})]
							})
						]
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "order-1 min-w-0 p-2 sm:p-3 lg:order-2 lg:p-4",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "mb-3 flex flex-wrap items-center justify-between gap-2 px-1 sm:px-2",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									className: "rounded-full border border-white/10 bg-slate-950/70 px-3 py-1.5 text-xs text-white/75",
									children: [
										selectedCityMeta.name,
										" · ",
										filteredParcels.length.toLocaleString("tr-TR"),
										" parsel"
									]
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									className: "flex items-center gap-2",
									children: [
										/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
											...
											className: `inline-flex items-center gap-2 rounded-full border px-3 py-2 text-xs font-semibold transition ${multiSelect ? "border-cyan-300/60 bg-cyan-300/15 text-cyan-100 shadow-lg shadow-cyan-500/10" : "border-white/15 bg-slate-950/80 text-white/80 hover:border-cyan-300/40 hover:text-white"}`,
											children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ListChecks, { className: "h-4 w-4" }), multiSelect ? "Çoklu Seçim Açık" : "Çoklu Seçim"]
										}),
										loading && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
											className: "text-xs text-white/45",
											children: "Görünen alan yükleniyor..."
										}),
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(FocusedGoogleParcelMap, {
								...
								center: selectedCityMeta.center,
								focusTarget
							}),
							multiSelect && selectedParcels.length > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "mt-4 rounded-2xl border border-cyan-200/20 bg-slate-950/95 p-4 shadow-2xl backdrop-blur-md",
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										className: "flex flex-wrap items-center justify-between gap-3",
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
											className: "text-sm font-semibold text-white",
											children: [selectedParcels.length, " parsel seçildi"]
										}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
											className: "mt-1 text-xs text-white/50",
											children: "Seçtiğin parsellerin statü ışığı haritada yanık kalır."
										})] }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
											...
											children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
												...
												children: [selectedTotal.toLocaleString("tr-TR"), " TL"]
											})]
										})]
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
										className: "mt-3 grid max-h-56 gap-2 overflow-auto pr-1 sm:grid-cols-2",
										children: selectedParcels.map((parcel) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
											...
											children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
												...
												children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
													...
													children: [
														parcel.city_name,
														" · ",
														parcel.tier === "premium" ? "Premium" : parcel.tier === "elite" ? "Elit" : "Dijital"
													]
												})]
											}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
												type: "button",
												onClick: () => removeSelected(parcel.id),
												className: "rounded-lg p-1.5 text-white/50 hover:bg-white/10 hover:text-white",
												"aria-label": `${parcel.parcel_number} seçimini kaldır`,
									...
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
										...
										className: "mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-300 px-4 py-3 text-sm font-bold text-slate-950 transition hover:bg-cyan-200 disabled:cursor-not-allowed disabled:opacity-50",
										disabled: !selectedParcelQuery,
										children: [
											/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ShoppingCart, { className: "h-4 w-4" }),
											"SEÇİLEN PARSELLERİ SATIN AL — ",
				...
				}), selectedParcel && !multiSelect && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ParcelDetailPanel, {
