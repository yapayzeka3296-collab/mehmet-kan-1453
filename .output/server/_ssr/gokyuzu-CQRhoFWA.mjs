...
//#region node_modules/.nitro/vite/services/ssr/assets/gokyuzu-CQRhoFWA.mjs
...
function GokyuzuPage() {
	...
	(0, import_react.useEffect)(() => {
		...
		const cameraTarget = new Vector3(initialWorldX, 2.5, initialWorldZ);
		...
		const clampGridRotation = () => {
			parcelGroup.rotation.x = MathUtils.clamp(parcelGroup.rotation.x, -1.05, 1.05);
		};
		const setZoom = (distance) => {
			camera.position.y = MathUtils.clamp(distance, 14, 90);
			camera.position.z = cameraTarget.z + MathUtils.clamp(distance, 18, 120);
			camera.lookAt(cameraTarget);
		};
		const onWheel = (event) => {
		...
		const onPointerMove = (event) => {
			if (!pointers.get(event.pointerId)) return;
			event.preventDefault();
			event.stopPropagation();
			pointers.set(event.pointerId, {
				x: event.clientX,
			...
			lastPointerX = event.clientX;
			lastPointerY = event.clientY;
			parcelGroup.rotation.y += dx * .008;
			parcelGroup.rotation.x += dy * .005;
			clampGridRotation();
		};
		...
		const loadCityParcels = async (cityIndex) => {
			...
			if (!cityNamesLoaded) {
				...
				cityCodes = (result.data ?? []).map((city) => ({
					name: city.name,
					code: city.code
				})).sort((a, b) => {
					return (specialProvinceNumbers[a.code] ?? Number(a.code)) - (specialProvinceNumbers[b.code] ?? Number(b.code));
				}).map((city) => city.code);
			...
			try {
				const result = await supabaseBrowser.from("parcel_map_public").select("id,parcel_number,status,price,tier,city_name,city_code,layer_number,sector_number,grid_x,grid_y").eq("city_code", cityCode).order("grid_y", { ascending: true }).order("grid_x", { ascending: true }).limit(1e3);
		...
		const updateVisibleParcels = async () => {
			...
			const safeColumn = MathUtils.clamp(centerColumn, 0, 359);
			const safeRow = MathUtils.clamp(centerRow, 0, 224);
			const minColumn = MathUtils.clamp(safeColumn - VISIBLE_X, 0, 359);
			const maxColumn = MathUtils.clamp(safeColumn + VISIBLE_X, 0, 359);
			const minRow = MathUtils.clamp(safeRow - VISIBLE_Z, 0, 224);
			const maxRow = MathUtils.clamp(safeRow + VISIBLE_Z, 0, 224);
