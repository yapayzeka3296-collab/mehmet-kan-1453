...
import { C as SphereGeometry, D as TextureLoader, E as StaticDrawUsage, O as Vector2, S as Scene, T as SpriteMaterial, _ as Plane, a as BufferGeometry, b as Raycaster, c as Group, d as LineSegments, f as MathUtils, g as PerspectiveCamera, h as MeshBasicMaterial, i as BufferAttribute, k as Vector3, l as InstancedMesh, m as Mesh, o as CanvasTexture, p as Matrix4, s as Color, t as WebGLRenderer, u as LineBasicMaterial, v as PlaneGeometry, w as Sprite, x as SRGBColorSpace, y as Ray } from "../_libs/three.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/gokyuzu-CCOUWDvh.mjs
	...
	(0, import_react.useEffect)(() => {
		...
		const onPointerMove = (event) => {
			...
			if (pointers.size === 2 && pinchDistance) {
				...
				pointer.y = -((focusY - rect.top) / rect.height) * 2 + 1;
				raycaster.setFromCamera(pointer, camera);
				if (raycaster.ray.intersectPlane(zoomPlane, zoomWorldBefore)) setZoom(current / (distance / pinchDistance), focusX, focusY);
				else setZoom(current / (distance / pinchDistance));
		...
		const loadAllWorldData = async () => {
			...
			for (let i = 0; i < allParcels.length; i += 1) {
				...
				if (parcel.status === "sold") baseColor.set(14427686);
			...
			for (let i = 0; i < allParcels.length; i += 1) {
				...
				if (ad?.is_active && ad.image_path) {
					...
					loader.load(adUrl, (texture) => {
						...
						const logo = new Sprite(material);
