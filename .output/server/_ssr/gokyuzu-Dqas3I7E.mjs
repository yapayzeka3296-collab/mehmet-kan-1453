...
import "./router-tKDwMhYs.mjs";
import { _ as SRGBColorSpace, a as BufferGeometry, b as Vector2, c as FogExp2, d as Line, f as LineBasicMaterial, g as Raycaster, h as PerspectiveCamera, i as BoxGeometry, l as Group, m as MeshBasicMaterial, n as WebGLRenderer, o as Color, p as Matrix4, s as DynamicDrawUsage, t as OrbitControls, u as InstancedMesh, v as Scene, x as Vector3, y as SphereGeometry } from "../_libs/three.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/gokyuzu-Dqas3I7E.mjs
...
function GokyuzuPage() {
	...
	(0, import_react.useEffect)(() => {
		...
		scene.fog = new FogExp2(589857, .008);
		...
		const geometry = new BoxGeometry(.92, .035, .92);
		...
		scene.add(mesh);
		const nodeGeometry = new SphereGeometry(.035, 6, 6);
		const nodeMaterial = new MeshBasicMaterial({
			color: 3718648,
			transparent: true,
			opacity: .9
		});
		const nodeMesh = new InstancedMesh(nodeGeometry, nodeMaterial, Math.min(VISIBLE_PARCELS, parcels.length));
		...
		for (let row = 0; row < GRID_ROWS; row += 2) {
			const points = [];
			for (let column = 0; column < GRID_COLUMNS; column += 1) points.push(new Vector3(parcelPosition(row * GRID_COLUMNS + column).x, parcelPosition(row * GRID_COLUMNS + column).y - .015, parcelPosition(row * GRID_COLUMNS + column).z));
			guideGroup.add(new Line(new BufferGeometry().setFromPoints(points), guideMaterial));
		}
		for (let column = 0; column < GRID_COLUMNS; column += 2) {
			const points = [];
			for (let row = 0; row < GRID_ROWS; row += 1) points.push(new Vector3(parcelPosition(row * GRID_COLUMNS + column).x, parcelPosition(row * GRID_COLUMNS + column).y - .012, parcelPosition(row * GRID_COLUMNS + column).z));
			guideGroup.add(new Line(new BufferGeometry().setFromPoints(points), guideMaterial));
		}
		...
		const updateVisibleWindow = () => {
			...
			for (let instance = 0; instance < mesh.count; instance += 1) {
				...
				color.set(parcel.status === "sold" ? 16751872 : parcel.status === "reserved" ? 10980346 : 3718648);
		...
		return () => {
			...
			guideGroup.children.forEach((child) => child.geometry.dispose());
			renderer.dispose();
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("main", {
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				"aria-label": "Parsel Dünyası 3D gökyüzü haritası"
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("header", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "Gökyüzündeki dijital parsel ağını sürükleyerek keşfet." })
