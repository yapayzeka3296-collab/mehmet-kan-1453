...
import { s as supabaseBrowser } from "./router-tgIdqXRD.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/gokyuzu-ChOV6oYJ.mjs
...
function GokyuzuPage() {
	...
	(0, import_react.useEffect)(() => {
		...
		const updateHover = (event) => {
			...
			const hit = raycaster.intersectObject(parcelGroup, true).map((entry) => entry.object).map((object) => findParcelObject(object) ?? (Number.isInteger(object.userData?.worldColumn) && Number.isInteger(object.userData?.worldRow) ? object : null)).find((object) => Boolean(object));
		...
		const onSelectPointerUp = async (event) => {
			...
			const loadedParcel = (parcelHit ? findParcelObject(parcelHit.object) ?? parcelHit.object : null)?.userData?.parcel;
			if (loadedParcel) {
