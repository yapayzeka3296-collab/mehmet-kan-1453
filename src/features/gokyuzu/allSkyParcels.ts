...
export async function loadAllSkyParcels(
  ...
  const first = await supabaseBrowser
    ...
    .select('id,parcel_number,status,price,tier,city_name,layer_number,sector_number,grid_x,grid_y', { count: 'exact' })
  ...
  for (let i = 0; i < offsets.length; i += PARALLEL_PAGES) {
    ...
    const pages = await Promise.all(
      batch.map((offset) =>
        supabaseBrowser
          .from('parcel_map_public')
          .select('id,parcel_number,status,price,tier,city_name,layer_number,sector_number,grid_x,grid_y')
          .order('city_name', { ascending: true })
          .order('layer_number', { ascending: true })
          .order('sector_number', { ascending: true })
