    ...
    <div className="space-y-4">
      ...
      <div className="panel overflow-auto p-5">
        <table className="w-full min-w-[800px] text-left text-xs">
          ...
          <tbody>{rows.map((row) => <tr key={row.id} onClick={() => setSelectedUserId(row.id)} className="cursor-pointer border-b border-border/50 hover:bg-accent/40" tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setSelectedUserId(row.id); }}><td className="p-2"><span className="inline-flex items-center gap-2"><UserRound className="h-3.5 w-3.5 text-muted-foreground" />{row.full_name || "Belirtilmemiş"}</span></td><td className="p-2">{row.email || "—"}</td><td className="p-2">{row.role || "—"}</td><td className="p-2">{row.created_at ? new Date(row.created_at).toLocaleString("tr-TR") : "—"}</td></tr>)}</tbody>
        </table>
