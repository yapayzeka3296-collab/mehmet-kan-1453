...
const musicMimeFromPath = (path: string | null) => {
...
export function ParcelDetailPanel({ parcel, onClose, onLocate }: Props) {
  ...
  async function handleMemorySave() {
    ...
    try {
      ...
      if (memoryMusicFile) {
        if (!memoryMusicFile.type.startsWith('audio/') && musicMimeType(memoryMusicFile) === 'application/octet-stream') throw new Error('Lütfen bir müzik dosyası seçin.'); if (memoryMusicFile.size > MAX_MUSIC_BYTES) throw new Error('Müzik en fazla 8 MB olabilir.');
