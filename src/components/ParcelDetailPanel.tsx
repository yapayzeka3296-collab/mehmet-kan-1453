...
const musicMimeTypeFromPath = (path: string | null) => {
...
export function ParcelDetailPanel({ parcel, onClose, onLocate }: Props) {
  ...
  const [memory, setMemory] = useState<Memory | null>(null); const [memoryPhotoUrl, setMemoryPhotoUrl] = useState<string | null>(null); const [memoryMusicUrl, setMemoryMusicUrl] = useState<string | null>(null); const [memoryMusicPreviewUrl, setMemoryMusicPreviewUrl] = useState<string | null>(null); const [memoryMusicError, setMemoryMusicError] = useState<string | null>(null);
  const [memoryNote, setMemoryNote] = useState(''); const [memoryFile, setMemoryFile] = useState<File | null>(null); const [memoryMusicFile, setMemoryMusicFile] = useState<File | null>(null); const [memoryIsPublic, setMemoryIsPublic] = useState(false); const [memoryMessage, setMemoryMessage] = useState<string | null>(null); const [editingMemory, setEditingMemory] = useState(false); const [memoryMusicPlaying, setMemoryMusicPlaying] = useState(false); const memoryAudioRef = useRef<HTMLAudioElement | null>(null);
  ...
  useEffect(() => { let cancelled = false; setMemoryLoading(true); setMemory(null); setMemoryPhotoUrl(null); setMemoryMusicUrl(null); setMemoryMusicPreviewUrl(null); setMemoryMusicError(null); setMemoryNote(''); setMemoryFile(null); setMemoryMusicFile(null); setMemoryIsPublic(false); setMemoryMessage(null); setEditingMemory(false); setMemoryMusicPlaying(false); if (memoryAudioRef.current) { memoryAudioRef.current.pause(); memoryAudioRef.current.removeAttribute('src'); memoryAudioRef.current.load(); } memoryAudioRef.current = null;
  ...
  function cancelMemoryEditor() { if (memoryAudioRef.current) memoryAudioRef.current.pause(); setMemoryMusicPlaying(false); setMemoryMessage(null); setMemoryMusicError(null); setMemoryFile(null); setMemoryMusicFile(null); setMemoryNote(memory?.note ?? ''); setMemoryIsPublic(memory?.is_public ?? false); setMemoryMusicPreviewUrl(null); setEditingMemory(false); }
  ...
  function handleMemoryMusicChange(event: ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0] ?? null; if (memoryMusicPreviewUrl) URL.revokeObjectURL(memoryMusicPreviewUrl); if (memoryAudioRef.current) { memoryAudioRef.current.pause(); memoryAudioRef.current.removeAttribute('src'); memoryAudioRef.current.load(); } setMemoryMusicPlaying(false); setMemoryMusicFile(file); setMemoryMusicError(null); setMemoryMessage(null); if (!file) { setMemoryMusicPreviewUrl(null); return; } const ext = (file.name.split('.').pop() || '').toLowerCase(); const contentType = musicMimeType(file); if (!MUSIC_EXTENSIONS.includes(ext)) { setMemoryMusicPreviewUrl(null); setMemoryMusicError('MP3, M4A, AAC, WAV, OGG veya WebM kullanın.'); return; } if (file.size > MAX_MUSIC_BYTES) { setMemoryMusicPreviewUrl(null); setMemoryMusicError('Müzik en fazla 8 MB olabilir.'); return; } const audio = document.createElement('audio'); if (!audio.canPlayType(contentType)) setMemoryMusicError('Bu ses formatı tarayıcıda doğrudan oynatılamıyor. MP3 kullanmanız önerilir.'); setMemoryMusicPreviewUrl(URL.createObjectURL(file)); }
  ...
  async function handleMemorySave() {
    ...
    try {
      ...
      if (memoryMusicFile) {
        ...
        const contentType = musicMimeType(memoryMusicFile); if (contentType === 'application/octet-stream') throw new Error('Lütfen desteklenen bir müzik dosyası seçin.');
        const audio = document.createElement('audio'); if (!audio.canPlayType(contentType)) throw new Error('Bu ses formatı tarayıcıda oynatılamıyor. MP3 kullanmanızı öneririm.');
        uploadedMusicPath = `${user.id}/${parcel.id}/memory-music-${Date.now()}.${ext}`; const { error } = await supabaseBrowser.storage.from('parcel-memories').upload(uploadedMusicPath, memoryMusicFile, { upsert: false, contentType, cacheControl: '3600' }); if (error) throw new Error(`Müzik yüklenemedi: ${error.message}`); nextMusicPath = uploadedMusicPath;
      }
      ...
      const photoPromise = nextMemory.photo_path ? supabaseBrowser.storage.from('parcel-memories').createSignedUrl(nextMemory.photo_path, 3600) : Promise.resolve({ data: null, error: null });
      const musicPromise = nextMemory.music_path ? supabaseBrowser.storage.from('parcel-memories').createSignedUrl(nextMemory.music_path, 3600) : Promise.resolve({ data: null, error: null });
  ...
  async function handleMemoryDelete() {
    ...
    try {
      ...
      if (memoryAudioRef.current) { memoryAudioRef.current.pause(); memoryAudioRef.current.removeAttribute('src'); memoryAudioRef.current.load(); } setMemory(null); setMemoryPhotoUrl(null); setMemoryMusicUrl(null); setMemoryMusicPreviewUrl(null); setMemoryMusicError(null); setMemoryMusicPlaying(false); setMemoryNote(''); setMemoryMusicFile(null); setMemoryFile(null); setEditingMemory(false); setMemoryMessage('Hatıran silindi.');
  ...
  useEffect(() => {
    ...
    if (!audio || !activeMusicUrl) { setMemoryMusicPlaying(false); return; }
    audio.pause();
    audio.currentTime = 0;
    audio.load();
  ...
  async function toggleMemoryPlayback() {
    const audio = memoryAudioRef.current;
    if (!audio || !activeMusicUrl) { setMemoryMusicError('Oynatılacak bir müzik bulunamadı.'); return; }
    setMemoryMusicError(null);
    if (!audio.paused) { audio.pause(); setMemoryMusicPlaying(false); return; }
    try {
      await audio.play();
