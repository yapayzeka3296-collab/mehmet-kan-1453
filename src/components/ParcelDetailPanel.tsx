...
import { X, ImagePlus, Pencil, ShoppingCart } from 'lucide-react';
...
export function ParcelDetailPanel({ parcel, onClose }: Props) {
  ...
  return (
    <aside className="fixed inset-0 z-[100] grid place-items-center bg-black/45 p-4 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-label="Parsel bilgileri">
      <div className="max-h-[92vh] w-full max-w-md overflow-auto rounded-2xl border border-cyan-300/20 bg-[#071a2d] p-5 shadow-2xl shadow-black/60 sm:p-6">
        ...
        <section className="mt-4 rounded-xl border border-cyan-300/15 bg-cyan-950/10 p-4" aria-label="Parsel bilgileri ve hatırası">
          ...
          {canBuy && <button type="button" onClick={handleBuy} className="btn-gold mt-4 flex h-11 w-full items-center justify-center gap-2 rounded-lg text-xs font-bold"><ShoppingCart className="h-4 w-4" /> SATIN AL</button>}
