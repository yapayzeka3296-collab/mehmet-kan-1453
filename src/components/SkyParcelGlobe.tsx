...
function rowTier(row: number): ParcelTier { if (row >= 8) return "premium"; if (row >= 6) return "elite"; return "digital"; }
function buildLayout(parcels: Parcel[]): LayoutCell[] { const result: LayoutCell[] = []; const used = new Set<string>(); for (const parcel of parcels) { const parsed = parseParcelCode(String(parcel.parcel_number ?? "")); if (parsed.sector === null || parsed.row === null) continue; const key = `${parsed.row}:${parsed.sector}`; if (!used.has(key)) { used.add(key); result.push({ parcel, sector: parsed.sector, row: parsed.row }); } } let cursor = 0; for (const parcel of parcels) { if (result.some((x) => x.parcel.id === parcel.id)) continue; while (used.has(`${Math.floor(cursor / SECTORS)}:${cursor % SECTORS}`)) cursor++; if (cursor >= ROWS * SECTORS) break; const row = Math.floor(cursor / SECTORS); const sector = cursor % SECTORS; used.add(`${row}:${sector}`); result.push({ parcel, sector, row }); cursor++; } return result; }
...
function selectVisibleCells(candidates: Cell[], yaw: number): Cell[] { const result: Cell[] = []; const facingTheta = Math.PI / 2 + yaw; for (let row = 0; row < ROWS; row++) { const quota = ROW_VISIBLE_COUNTS[row] ?? 0; const rowCandidates = candidates.filter((cell) => cell.row === row); if (rowCandidates.length === 0 || quota === 0) continue; const halfSpan = Math.min(1.48, 0.78 + row * 0.078); const rowOffset = row % 2 === 0 ? -0.018 : 0.018; const targets = Array.from({ length: quota }, (_, index) => { const t = quota === 1 ? 0.5 : index / (quota - 1); return facingTheta - halfSpan + t * halfSpan * 2 + rowOffset; }); const remaining = new Set(rowCandidates); for (const target of targets) { let best: Cell | null = null; let bestDistance = Infinity; for (const candidate of remaining) { const theta = -Math.PI + (2 * Math.PI * (candidate.sector + 0.5)) / SECTORS; const distance = angularDistance(theta, target); if (distance < bestDistance) { best = candidate; bestDistance = distance; } } if (best) { result.push(best); remaining.delete(best); } } } return result.slice(0, VISIBLE_CELLS); }
...
export function SkyParcelGlobe({ parcels, selectedId, onSelect }: Props) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const item of layoutRef.current) { const theta0 = -Math.PI + (2 * Math.PI * item.sector) / SECTORS; const theta1 = -Math.PI + (2 * Math.PI * (item.sector + 1)) / SECTORS; const phi0 = PHI_MIN + (PHI_MAX - PHI_MIN) * (item.row / ROWS); const phi1 = PHI_MIN + (PHI_MAX - PHI_MIN) * ((item.row + 1) / ROWS); const center3 = rotatePoint(spherePoint((theta0 + theta1) / 2, (phi0 + phi1) / 2), s.yaw, s.pitch); if (center3.z <= 0.015) continue; const corners = [spherePoint(theta0, phi0), spherePoint(theta1, phi0), spherePoint(theta1, phi1), spherePoint(theta0, phi1)].map((p) => project(rotatePoint(p, s.yaw, s.pitch), radius, cx, cy)); const center = project(center3, radius, cx, cy); if (center.x < -radius || center.x > width + radius || center.y < -radius || center.y > height + radius) continue; candidates.push({ parcel: item.parcel, sector: item.sector, row: item.row, depth: center3.z, center, points: corners, tier: rowTier(item.row) }); }
      const visible = selectVisibleCells(candidates, s.yaw).sort((a, b) => a.depth - b.depth); hitRef.current = visible;
