...
].map(([name,slug,lat,lng], i) => ({ code: String(i + 1).padStart(2, "0"), name: String(name), slug: String(slug), center: { lat: Number(lat), lng: Number(lng) } }));
...
export function LegacySkyMapView() {
  const [selectedCity, setSelectedCity] = useState(DEFAULT_CITY.code);
  ...
  const city = CITIES.find(c => c.code === selectedCity) ?? DEFAULT_CITY;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data } = await supabaseBrowser.rpc("parcels_in_view", { p_city_slug: city.slug, p_min_lat: city.center.lat - 1, p_min_lng: city.center.lng - 1, p_max_lat: city.center.lat + 1, p_max_lng: city.center.lng + 1 });
      if (active) { setParcels((data ?? []) as MapParcel[]); setLoading(false); }
    };
    void load();
    return () => { active = false; };
  }, [city.code, city.center.lat, city.center.lng, city.slug]);

  const chooseCity = (code: string) => { setSelectedCity(code); setLayerFilter(null); setSectorFilter(null); };
  const toggle = (id: string) => setSelectedIds(prev => { const next = new Set(prev); next.has(id) ? next.delete(id) : next.add(id); return next; });
  const layers = Array.from(new Set(parcels.map(p => p.layer_number).filter((x): x is number => typeof x === "number"))).sort((a,b) => a-b);
  const sectors = Array.from(new Set((layerFilter === null ? parcels : parcels.filter(p => p.layer_number === layerFilter)).map(p => p.sector_number).filter((x): x is number => typeof x === "number"))).sort((a,b) => a-b);
  ...
  return <div className="min-h-screen bg-slate-950 text-white">
    <main className="mx-auto max-w-[1800px] px-3 py-3 sm:px-5 lg:px-8">
      <section className="grid overflow-hidden rounded-3xl border border-sky-200/15 bg-slate-900/70 shadow-2xl shadow-black/30 lg:grid-cols-[280px_1fr]">
        ...
        <div className="order-1 min-w-0 p-2 sm:p-3 lg:order-2 lg:p-4"><div className="mb-3 flex items-center justify-between px-1"><div className="rounded-full border border-white/10 bg-slate-950/70 px-3 py-1.5 text-xs text-white/75">{city.name} · {filteredParcels.length.toLocaleString("tr-TR")} parsel</div>{loading && <span className="text-xs text-white/45">Parseller yükleniyor...</span>}</div><SkyParcelMap parcels={filteredParcels} selectedId={null} selectedIds={selectedIds} multiSelect onSelect={() => undefined} onToggleSelect={toggle} center={city.center} />{selectedParcels.length > 0 && <div className="mt-4 rounded-2xl border border-cyan-200/20 bg-slate-950/95 p-4"><div className="flex items-center justify-between"><div><p className="text-sm font-semibold">{selectedParcels.length} parsel seçildi</p><p className="mt-1 text-xs text-white/50">Haritada bir parsele tekrar dokunarak seçimini kaldırabilirsin.</p></div><div className="text-right"><p className="text-xs text-white/50">Toplam</p><p className="text-lg font-bold text-cyan-100">{total.toLocaleString("tr-TR")} TL</p></div></div><div className="mt-3 grid max-h-56 gap-2 overflow-auto sm:grid-cols-2">{selectedParcels.map(p => <div key={p.id} className="flex items-center justify-between rounded-xl border border-white/10 px-3 py-2"><div><p className="text-sm font-medium">{p.parcel_number}</p><p className="text-xs text-white/45">{p.city_name} · {p.tier}</p></div><button type="button" onClick={() => setSelectedIds(prev => { const n = new Set(prev); n.delete(p.id); return n; })} className="rounded-lg p-1.5 text-white/50 hover:bg-white/10" aria-label="Parsel seçimini kaldır"><X className="h-4 w-4"/></button></div>)}</div><button type="button" onClick={() => { if (selectedParcels.length) window.location.href = `/parsel-satin-al?parcels=${selectedParcels.map(p => p.id).join(",")}`; }} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-300 px-4 py-3 text-sm font-bold text-slate-950"><ShoppingCart className="h-4 w-4"/>SEÇİLEN PARSELLERİ SATIN AL — {total.toLocaleString("tr-TR")} TL</button></div>}</div>
      </section>
    </main>
  </div>;
}
