  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      const fallback = () => {
        ...
        el.innerHTML =
          '<div style="width:min(62vw,520px);aspect-ratio:1;border-radius:50%;background:radial-gradient(circle at 32% 28%,#38bdf8,#0b4776 38%,#031329 72%);box-shadow:0 0 80px rgba(56,189,248,.22),inset -28px -22px 70px rgba(0,0,0,.72)"></div>';
      ...
      try {
        ...
        const makeTexture = (renderer: InstanceType<typeof THREE.WebGLRenderer>, clouds = false) => {
          ...
          if (clouds) {
            ...
            const shapes = [
              [[.07, .2], [.18, .12], [.28, .22], [.24, .4], [.18, .48], [.1, .4]],
              [[.28, .56], [.38, .52], [.4, .68], [.32, .88], [.25, .72]],
              [[.4, .18], [.52, .12], [.6, .25], [.55, .35], [.44, .3]],
              [[.48, .42], [.6, .38], [.65, .5], [.58, .68], [.5, .58]],
              [[.62, .18], [.78, .14], [.88, .27], [.82, .4], [.68, .34]],
              [[.76, .48], [.9, .44], [.96, .58], [.84, .64]],
            ];
            ...
            for (const points of shapes) {
              ...
              points.forEach(([x, y], i) =>
                i ? ctx.lineTo(x * 1024, y * 512) : ctx.moveTo(x * 1024, y * 512),
              );
              ctx.closePath();
              ctx.fill();
              ctx.stroke();
        ...
        scene.add(new THREE.AmbientLight(0x6e88ad, .5));
        ...
        const r = RADIUS * mobile;
        const geo = new THREE.SphereGeometry(r, 96, 96);
        const mat = new THREE.MeshPhongMaterial({ map: earthTex, shininess: 8, specular: new THREE.Color(0x1c3550) });
        const earth = new THREE.Mesh(geo, mat);
        scene.add(earth);

        const cgeo = new THREE.SphereGeometry(r * 1.014, 72, 72);
        const cmat = new THREE.MeshPhongMaterial({ color: 0xffffff, alphaMap: cloudTex, transparent: true, opacity: .42, depthWrite: false });
        const clouds = new THREE.Mesh(cgeo, cmat);
        scene.add(clouds);

        const ageo = new THREE.SphereGeometry(r * 1.085, 72, 72);
        const amat = new THREE.MeshBasicMaterial({ color: 0x4da6ff, transparent: true, opacity: .13, side: THREE.BackSide, blending: THREE.AdditiveBlending, depthWrite: false });
        ...
        const smat = new THREE.PointsMaterial({ color: 0xffffff, size: .048 });
