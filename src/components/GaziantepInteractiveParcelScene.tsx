  ...
  return (
    <section className="mt-5 overflow-hidden rounded-3xl border border-amber-200/20 bg-slate-950/80 p-2 shadow-2xl sm:p-3">
      <div className="mb-2 px-2 py-1">
        ...
        <p className="mt-1 text-[10px] text-white/50">MySkyParcel kayıtları görselin üzerine yerleştirilir. Yeni parsel üretilmez; mevcut parseller sürüklenebilir.</p>
      </div>
      <div ref={containerRef} className="relative aspect-[3/2] overflow-hidden rounded-2xl bg-slate-900 select-none touch-none">
        <img src={CITY_IMAGES.GZT} alt="Gaziantep şehir görünümü" className="absolute inset-0 h-full w-full object-cover" draggable={false} />
        ...
        {gaziantepParcels.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="rounded-full border border-white/20 bg-black/60 px-4 py-2 text-xs text-white/80 backdrop-blur">Gaziantep için henüz parsel kaydı bulunamadı.</div>
          </div>
        )}
        {activeId && (
          <div className="pointer-events-none absolute bottom-3 left-1/2 z-30 -translate-x-1/2 rounded-full border border-amber-200/20 bg-black/60 px-3 py-1 text-[10px] text-white/80 backdrop-blur">Parsel taşınıyor</div>
        )}
      </div>
    </section>
  );
