...
export function CityParcelPage({ slug }: { slug: string }) {
  ...
  const [city, setCity] = useState<City | null>(null); const [parcels, setParcels] = useState<Parcel[]>([]);
  ...
  useEffect(() => { let active = true; setLoading(true); setCity(null); setParcels([]); setSelectedIds(new Set()); setImageFailed(false);
    const load = async () => { if (!supabaseBrowser) { if (active) setLoading(false); return; }
      ...
      if (!active) return; if (!error) setParcels(selectVisibleCityParcels(((data ?? []) as Parcel[]).filter((p) => p.city_slug === slug))); setLoading(false);
