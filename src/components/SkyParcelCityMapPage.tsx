...
// Explicit concentric assignment: nearest 8 = Premium, next 22 = Elite, outer 30 = Digital.
...
function ParcelLines({ parcels, selectedIds, onToggle }: { parcels: VisibleParcel[]; selectedIds: Set<string>; onToggle: (p: VisibleParcel) => void }) {
  ...
  return <div className="absolute inset-0 z-20 grid touch-manipulation" style={{ gridTemplateColumns: `repeat(${COLS},minmax(0,1fr))`, gridTemplateRows: `repeat(${ROWS},minmax(0,1fr))` }} aria-label="Gökyüzü parsel çizgileri">
    {VISIBLE_SLOTS.map(slot => { const p = slotParcel.get(`${slot.x}:${slot.y}`); if (!p) return <span key={`${slot.x}:${slot.y}`} className="border border-cyan-200/10" aria-hidden />; const selected = selectedIds.has(p.id); const rgb = TIER_COLOR[p.tier]; return <button key={p.id} type="button" disabled={p.status !== "available"} aria-label={`${p.parcel_number} ${p.tier}`} title={`${p.parcel_number} · ${p.tier}`} onClick={e => { e.stopPropagation(); onToggle(p); }} className="relative min-h-0 min-w-0 border transition-[background,border,box-shadow] duration-100 disabled:cursor-not-allowed" style={{ borderColor: selected ? "rgba(255,244,176,.98)" : `rgba(${rgb},.55)`, background: selected ? "rgba(255,211,92,.48)" : `rgba(${rgb},.055)`, boxShadow: selected ? "inset 0 0 0 1px rgba(255,244,176,.95),0 0 12px rgba(255,211,92,.75)" : `inset 0 0 0 1px rgba(${rgb},.08)` }}>{selected && <span className="pointer-events-none absolute left-1/2 top-1/2 z-30 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap rounded bg-slate-950/95 px-1.5 py-0.5 text-[8px] font-semibold text-white shadow-lg">{p.parcel_number}</span>}</button>; })}
  </div>;
}
...
export function SkyParcelCityMapPage({ slug }: { slug: string }) {
  ...
  useEffect(() => { if (!city) return; let alive = true; setLoading(true); Promise.all((Object.keys(TIER_COUNTS) as ParcelTier[]).map(t => loadWindow(city.slug, t, TIER_COUNTS[t]))).then(groups => { if (!alive) return; setParcels(groups.flat()); setLoading(false); }).catch(() => { if (alive) setLoading(false); }); return () => { alive = false; }; }, [city?.slug]);
  ...
  const selected = useMemo(() => readParcelCart().filter(p => selectedIds.has(p.id)), [selectedIds]); const total = selected.reduce((sum, p) => sum + Number(p.tier_price), 0); const buy = () => { if (authLoading || !selected.length) return; const ids = selected.map(p => p.id).join(","); window.location.href = user ? `/parsel-satin-al?parcels=${ids}` : `/giris?redirect=${encodeURIComponent(`/parsel-satin-al?parcels=${ids}`)}`; };
