...
export function SkyParcelCityMapPage({ slug }: { slug: string }) {
  ...
  const loadWindow = async (citySlug: string) => { if (!supabaseBrowser) return [] as VisibleParcel[]; const { data, error } = await supabaseBrowser.rpc("parcels_in_view", { p_city_slug: citySlug, p_min_lat: -90, p_min_lng: -180, p_max_lat: 90, p_max_lng: 180 }); if (error) throw error; return normalizeCityParcels((data ?? []) as VisibleParcel[]); };
  useEffect(() => { if (!city) return; let alive = true; setLoading(true); loadWindow(city.slug).then(rows => { if (!alive) return; setParcels(rows); setLoading(false); }).catch(() => { if (alive) setLoading(false); }); return () => { alive = false; }; }, [city?.slug]);
  const visible = useMemo(() => parcels.slice(0, VISIBLE_SLOTS.length), [parcels]);
  const soldCount = useMemo(() => visible.filter(p => p.status === "sold").length, [visible]);
