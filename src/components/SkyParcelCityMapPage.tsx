...
function buildSlots(): Slot[] {
  ...
  for (let y = 2; y < ROWS - 2; y += 3) {
    for (let x = 1; x < COLS - 1; x += 3) result.push({ x, y });
  }
...
function SkyBackground() {
  return <div aria-hidden className="pointer-events-none absolute inset-0 bg-[#020914]">
    <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_42%,rgba(42,150,220,.28),transparent_35%),radial-gradient(circle_at_75%_75%,rgba(126,70,210,.18),transparent_30%),linear-gradient(145deg,#020711,#071a2d_52%,#01040b)]" />
    <div className="absolute inset-0 opacity-65" style={{ backgroundImage: "radial-gradient(circle,rgba(255,255,255,.85) .7px,transparent .9px)", backgroundSize: "29px 29px" }} />
    <div className="absolute inset-0 opacity-60" style={{ backgroundImage: "linear-gradient(rgba(76,224,255,.24) 1px,transparent 1px),linear-gradient(90deg,rgba(76,224,255,.24) 1px,transparent 1px)", backgroundSize: "32px 32px" }} />
    <div className="absolute inset-[-20%] opacity-50" style={{ backgroundImage: "linear-gradient(rgba(177,108,255,.16) 1px,transparent 1px),linear-gradient(90deg,rgba(177,108,255,.16) 1px,transparent 1px)", backgroundSize: "160px 160px", transform: "perspective(700px) rotateX(52deg) scale(1.45)", transformOrigin: "50% 55%", maskImage: "linear-gradient(to bottom,transparent,black 25%,black 80%,transparent)" }} />
  </div>;
}
...
function ParcelLines({ parcels, selectedIds, onToggle }: { parcels: VisibleParcel[]; selectedIds: Set<string>; onToggle: (p: VisibleParcel) => void }) {
  ...
  return <div className="absolute inset-0 z-20 grid touch-manipulation" style={{ gridTemplateColumns: `repeat(${COLS},minmax(0,1fr))`, gridTemplateRows: `repeat(${ROWS},minmax(0,1fr))` }} aria-label="Gökyüzü parsel çizgileri">
    {VISIBLE_SLOTS.map(slot => {
      ...
      return <button key={p.id} type="button" disabled={p.status !== "available"} aria-label={`${p.parcel_number} ${p.tier}`} title={`${p.parcel_number} · ${p.tier}`} onClick={e => { e.stopPropagation(); onToggle(p); }} className="relative min-h-0 min-w-0 border transition-[background,border,box-shadow] duration-100 disabled:cursor-not-allowed" style={{ borderColor: selected ? "rgba(255,244,176,.98)" : `rgba(${rgb},.55)`, background: selected ? "rgba(255,211,92,.48)" : `rgba(${rgb},.055)`, boxShadow: selected ? "inset 0 0 0 1px rgba(255,244,176,.95),0 0 12px rgba(255,211,92,.75)" : `inset 0 0 0 1px rgba(${rgb},.08)` }}>
        <span className="pointer-events-none absolute inset-[18%] rounded-sm" style={{ background: selected ? "rgba(255,211,92,.55)" : `rgba(${rgb},.12)` }} />
        {selected && <span className="pointer-events-none absolute left-1/2 top-1/2 z-30 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap rounded bg-slate-950/95 px-1.5 py-0.5 text-[8px] font-semibold text-white shadow-lg">{p.parcel_number}</span>}
      </button>;
    })}
  </div>;
}
...
export function SkyParcelCityMapPage({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    const loadCity = async () => {
      ...
      if (alive) setCity(data as City | null);
    };
    void loadCity();
    return () => { alive = false; };
  }, [slug]);
