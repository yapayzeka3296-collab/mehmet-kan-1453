    ...
    <header className="sticky top-0 z-40 h-[80px] w-full border-b border-[#1E293B] bg-[#050B1A]/90 shadow-lg shadow-black/20 backdrop-blur-md">
      <div className="mx-auto flex h-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-3 xl:flex 2xl:gap-4">
          <Link to="/pazar-yeri" aria-label="Pazar Yeri" title="Pazar Yeri" className="flex items-center gap-1.5 p-2 text-xs font-medium text-slate-300 transition-colors duration-200 hover:text-[#D4AF37]"><Store className="h-5 w-5" /> <span>PAZAR YERİ</span></Link>
      ...
      {open && <><div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm xl:hidden" onClick={closeMenu} aria-hidden="true" /><aside ref={drawerRef} id="mobile-navigation" role="dialog" aria-modal="true" aria-label="Site menüsü" className="fixed top-0 right-0 z-50 flex h-screen w-[min(320px,calc(100vw-24px))] flex-col overflow-hidden border-l border-[#1E293B] bg-[#050B1A] p-6 xl:hidden"><div className="min-h-0 flex-1 overflow-y-auto"><div className="flex items-center justify-between border-b border-[#1E293B] pb-6"><span className="text-lg font-bold text-white">Menü</span><button ref={closeButtonRef} type="button" onClick={closeMenu} aria-label="Menüyü kapat" className="p-1 text-slate-400 hover:text-white"><X className="h-6 w-6" /></button></div><nav aria-label="Mobil site navigasyonu" className="mt-6 flex flex-col gap-4 pb-6">{visibleNavLinks.map((item) => <Link key={item.to} to={item.to} onClick={closeMenu} className="text-base font-medium text-slate-300 transition-colors duration-200 hover:text-[#D4AF37]" activeProps={{ className: "font-semibold text-[#D4AF37]" }}>{item.label}</Link>)}<Link to="/pazar-yeri" onClick={closeMenu} className="text-base font-medium text-slate-300 transition-colors duration-200 hover:text-[#D4AF37]">Pazar Yeri</Link></nav></div><div className="shrink-0 flex flex-col gap-3 border-t border-[#1E293B] bg-[#050B1A] pt-4"><Link to="/giris" onClick={closeMenu} className="w-full rounded-lg border border-slate-700 py-2.5 text-center text-sm font-medium text-slate-200 transition-colors duration-200 hover:border-[#D4AF37] hover:text-[#D4AF37]">Giriş Yap</Link><Link to="/kayit-ol" onClick={closeMenu} className="w-full rounded-lg bg-[#D4AF37] py-2.5 text-center text-sm font-semibold text-black transition-colors duration-200 hover:bg-[#c29f2e]">Üye Ol</Link></div></aside></>}
    </header>
  );
}
