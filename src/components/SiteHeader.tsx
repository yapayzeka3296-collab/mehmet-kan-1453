...
const NAV_LINKS = [
  { label: "Ana Sayfa", to: "/" },
  { label: "MySkyParcel Nedir?", to: "/nasil-calisir" },
  { label: "Gökyüzü Haritası", to: "/gokyuzu-haritasi" },
  { label: "Koleksiyonum", to: "/parsellerim", requiresAuth: true },
  { label: "Dijital Sertifika", to: "/paketler" },
  { label: "Hakkımızda", to: "/hakkimizda" },
  { label: "SSS", to: "/hakkimizda", hash: "sss" },
...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 h-[80px] w-full border-b border-[#1E293B] bg-[color:var(--background)]/90 shadow-lg shadow-black/20 backdrop-blur-md">
      <div className="mx-auto flex h-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav aria-label="Ana navigasyon" className="hidden items-center gap-3 xl:flex 2xl:gap-5">
          {visibleNavLinks.map((item) => <Link key={`${item.label}-${item.to}`} to={item.to} hash={"hash" in item ? item.hash : undefined} search={item.to === "/gokyuzu-haritasi" ? { city: "istanbul" } : undefined} className="relative whitespace-nowrap text-xs font-medium text-slate-300 transition-colors duration-200 hover:text-[#D4AF37] 2xl:text-sm" activeProps={{ className: "text-[#D4AF37] after:absolute after:bottom-[-6px] after:left-0 after:h-[2px] after:w-full after:bg-[#D4AF37]" }}>{item.label}</Link>)}
