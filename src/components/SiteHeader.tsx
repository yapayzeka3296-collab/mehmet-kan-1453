    ...
    <header className="sticky top-0 z-40 h-[80px] w-full border-b border-[#1E293B] bg-[#050B1A]/90 shadow-lg shadow-black/20 backdrop-blur-md">
      ...
      {open && (
        <>
          ...
          <aside ref={drawerRef} id="mobile-navigation" role="dialog" aria-modal="true" aria-label="Site menüsü" className="fixed inset-y-0 right-0 z-50 flex h-[100dvh] max-h-[100dvh] w-[min(320px,calc(100vw-24px))] flex-col overflow-hidden border-l border-[#1E293B] bg-[#050B1A] p-6 pb-[calc(1rem+env(safe-area-inset-bottom))] xl:hidden">
            <div className="min-h-0 flex-1 overflow-y-auto">
              ...
              <nav aria-label="Mobil site navigasyonu" className="mt-6 flex flex-col gap-4 pb-4">
                ...
                {isAdmin && <Link to="/yonetim" onClick={closeMenu} className="flex items-center gap-2 text-base font-semibold text-[#D4AF37] transition-colors duration-200 hover:text-white"><ShieldCheck className="h-5 w-5" /> Yönetim</Link>}
                <Link to="/pazar-yeri" onClick={closeMenu} className="text-base font-medium text-slate-300 transition-colors duration-200 hover:text-[#D4AF37]">Pazar Yeri</Link>
              </nav>
            </div>
            <div className="shrink-0 flex flex-col gap-2 border-t border-[#1E293B] bg-[#050B1A] pt-3">
              {isAuthenticated ? (
                <>
                  <Link to="/panelim" onClick={closeMenu} className="w-full rounded-lg border border-slate-700 py-2.5 text-center text-sm font-medium text-slate-200 transition-colors duration-200 hover:border-[#D4AF37] hover:text-[#D4AF37]">Panelim</Link>
                  {isAdmin && <Link to="/yonetim" onClick={closeMenu} className="w-full rounded-lg border border-[#D4AF37] py-2.5 text-center text-sm font-semibold text-[#D4AF37]">Yönetim Paneli</Link>}
                  <button type="button" onClick={() => void handleSignOut()} className="w-full rounded-lg bg-[#D4AF37] py-2.5 text-center text-sm font-semibold text-black transition-colors duration-200 hover:bg-[#c29f2e]">Çıkış Yap</button>
                </>
              ) : (
                <>
