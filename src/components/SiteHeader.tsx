...
export function SiteHeader() {
  ...
  // Mobil menü açıkken arka planın (sitenin) kaymasını tamamen durdurur
  useEffect(() => {
    if (open) {
  ...
  return (
    <header className="sticky top-0 z-40 h-[80px] w-full border-b border-[#1E293B] bg-[#050B1A]/90 backdrop-blur-md shadow-lg shadow-black/20">
        ...
        <nav className="hidden lg:flex items-center gap-5 xl:gap-7">
          {NAV_LINKS.map((item) => (
            <Link
              key={item.to}
              to={item.to}
              className="relative text-sm font-medium text-slate-300 transition-colors duration-200 hover:text-[#D4AF37]"
              activeProps={{
      ...
      {/* MOBILE SLIDE-IN MENU OVERLAY & DRAWER */}
      {/* 1. Arka Plan Karartma (Backdrop) */}
      <div
      ...
      <aside
        className={`fixed top-0 right-0 z-50 h-screen w-[280px] sm:w-[320px] bg-[#050B1A] border-l border-[#1E293B] p-6 flex flex-col justify-between overflow-y-auto transform transition-transform duration-300 ease-in-out lg:hidden ${
