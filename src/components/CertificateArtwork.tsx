...
export function CertificateArtwork({ tier, name, parcelCode, certificateNumber, issuedAt, cityName }: CertificateArtworkProps) {
  const meta = META[tier]; const displayName = name?.trim() || 'Ad Soyad'; const [downloading, setDownloading] = useState(false);
  const downloadCertificate = useCallback(async () => {
    if (!certificateNumber) return; setDownloading(true);
    try {
      ...
      const blob = new Blob([svg], { type: 'image/svg+xml;charset=utf-8' }); const url = URL.createObjectURL(blob); const anchor = document.createElement('a');
      anchor.href = url; anchor.download = `${certificateNumber}-${tier}.svg`; document.body.appendChild(anchor); anchor.click(); anchor.remove(); URL.revokeObjectURL(url);
    } catch (error) { console.error('Certificate download failed', error); } finally { setDownloading(false); }
