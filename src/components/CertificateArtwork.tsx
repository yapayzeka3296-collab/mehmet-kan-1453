...
const LAYOUT = { name: { left: 24, width: 52, top: 43 }, coordinates: { left: 29, width: 42, top: 49.2 }, parcel: { left: 76.5, width: 18, top: 47 }, date: { left: 76.5, width: 18, top: 54 }, number: { left: 76.5, width: 18, top: 61 }, city: { left: 30, width: 40, top: 82 }, qr: { right: 6.2, top: 65.2, size: 10.5 } } as const;
...
async function render(tier: Tier, name: string, parcel: string, date: string, number: string, city: string, latitude?: number | null, longitude?: number | null, qrVersion?: string | null) {
  ...
  x.font = `600 ${nameSize}px Georgia,serif`; x.fillStyle = "#d5ad4a"; x.shadowColor = "rgba(0,0,0,.45)"; x.shadowBlur = c.width * .002; x.fillText(name || "Ad Soyad", c.width * ((LAYOUT.name.left + LAYOUT.name.width / 2) / 100), c.height * (LAYOUT.name.top / 100)); x.shadowBlur = 0;
  const coordinate = coordinateText(latitude, longitude); x.font = `600 ${c.width * .009}px Arial,sans-serif`; x.fillStyle = "#27384a"; x.fillText(coordinate, c.width * ((LAYOUT.coordinates.left + LAYOUT.coordinates.width / 2) / 100), c.height * (LAYOUT.coordinates.top / 100));
  const valueX = c.width * ((LAYOUT.parcel.left + LAYOUT.parcel.width / 2) / 100); x.fillText(parcel || "—", valueX, c.height * (LAYOUT.parcel.top / 100)); x.fillText(date || "—", valueX, c.height * (LAYOUT.date.top / 100)); x.fillText(number || "—", valueX, c.height * (LAYOUT.number.top / 100));
...
export function CertificateArtwork({ tier, name, parcelCode, certificateNumber, issuedAt, cityName, latitude, longitude }: Props) {
  ...
  return <article className="relative mx-auto w-full overflow-hidden rounded-xl bg-black shadow-2xl" aria-label={LABEL[tier]}><div className="relative"><img src={templateSrc} alt={LABEL[tier]} onError={(event) => { const img = event.currentTarget; if (img.src.endsWith(CERTIFICATE_TEMPLATE_FALLBACK)) return; img.src = CERTIFICATE_TEMPLATE_FALLBACK; }} className="block aspect-[1600/1067] h-auto w-full object-cover"/><div className="pointer-events-none absolute inset-0"><div className="absolute left-[24%] top-[43%] flex h-[8%] w-[52%] items-center justify-center text-center"><span className="font-serif text-[clamp(13px,2.1vw,28px)] font-semibold italic leading-none text-[#d5ad4a] drop-shadow-[0_1px_2px_rgba(0,0,0,.45)]">{displayName}</span></div><div className="absolute left-[29%] top-[49.2%] flex h-[4.5%] w-[42%] items-center justify-center text-center"><span className="text-[clamp(5px,.7vw,11px)] font-semibold leading-none text-[#27384a]">{coordinates}</span></div><div className="absolute left-[76.5%] top-[47%] w-[18%] text-center text-[clamp(6px,.85vw,13px)] font-semibold leading-tight text-[#27384a]">{parcelCode || "—"}</div><div className="absolute left-[76.5%] top-[54%] w-[18%] text-center text-[clamp(6px,.85vw,13px)] font-semibold leading-tight text-[#27384a]">{dateTR(issuedAt)}</div><div className="absolute left-[76.5%] top-[61%] w-[18%] text-center text-[clamp(6px,.85vw,13px)] font-semibold leading-tight text-[#27384a]">{certificateNumber || "—"}</div>{cityName && <div className="absolute left-[30%] top-[82%] w-[40%] text-center text-[clamp(6px,.75vw,12px)] font-semibold text-white/85">{cityName}</div>}{verificationQr && <img key={verificationQr} src={verificationQr} alt="Sertifika doğrulama QR kodu" className="absolute right-[6.2%] top-[65.2%] h-[10.5%] w-[10.5%] rounded bg-white p-1" onError={(event) => { event.currentTarget.style.display = "none"; }} />}</div></div>{certificateNumber && <div className="relative z-10 flex flex-wrap justify-end gap-2 border-t border-white/10 bg-[#06162d] p-2"><button type="button" onClick={() => void download("png")} disabled={busy}>PNG</button><button type="button" onClick={() => void download("jpg")} disabled={busy}>JPG</button><button type="button" onClick={() => void download("pdf")} disabled={busy}>PDF / YAZDIR</button></div>}{busy && <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/60 text-sm font-semibold text-gold">SERTİFİKA HAZIRLANIYOR...</div>}{error && <p className="absolute bottom-14 left-3 right-3 z-20 rounded bg-red-950/90 p-2 text-center text-xs text-red-200">{error}</p>}</article>;
}
