...
function cornerIcon(maps: Maps, color: string) {
  const cached = cornerIconCache.get(color);
  ...
  cornerIconCache.set(color, icon);
  return icon;
}

function buildSquareGridLayout(center: CityCenter, total: number, innerRadius: number, outerRadius: number): GridCell[] {
  if (total <= 0) return [];
  const cosLat = Math.max(Math.cos((center.lat * Math.PI) / 180), 0.2);
  let gridSize = Math.max(2, Math.ceil(Math.sqrt(total)));
  let candidates: Array<{ x0: number; y0: number; x1: number; y1: number; ring: number }> = [];
  const buildCandidates = (size: number) => {
    ...
    for (let row = 0; row < size; row += 1) {
      for (let col = 0; col < size; col += 1) {
        ...
        next.push({ x0, y0, x1, y1, ring: Math.max(Math.abs(cx), Math.abs(cy)) });
      }
    }
    return next;
  };
  ...
  return candidates.map(({ x0, y0, x1, y1 }) => {
    const toPoint = (x: number, y: number) => ({ lat: center.lat + y, lng: center.lng + x / cosLat });
    return {
...
function geometrySignature(parcels: ParcelWithGeometry[], center: CityCenter) {
  const idsByTier = ["digital", "elite", "premium"].map((tier) =>
    parcels.filter((parcel) => parcel.tier === tier).map((parcel) => parcel.id).join("|")
  );
  return `${center.lat.toFixed(7)}:${center.lng.toFixed(7)}::${idsByTier.join("::")}`;
}
...
export function GoogleParcelMap({ parcels, selectedId, selectedIds = new Set<string>(), multiSelect = false, onSelect, onToggleSelect, onViewportChange, center }: Props) {
  ...
  const renderedGeometrySignatureRef = useRef("");
  const hoveredRef = useRef<Set<string>>(new Set());
  const [error, setError] = useState<string | null>(null);
  const [ready, setReady] = useState(false);

  useEffect(() => { selectRef.current = onSelect; }, [onSelect]);
  useEffect(() => { toggleRef.current = onToggleSelect; }, [onToggleSelect]);
  ...
  useEffect(() => {
    ...
    const next = new Map<string, ParcelWithGeometry>();
    parcels.forEach((parcel) => next.set(parcel.id, parcel));
    parcelByIdRef.current = next;
  }, [parcels]);
  ...
  const rebuildGeometry = useCallback((maps: Maps) => {
    ...
    configs.forEach((cfg) => {
      const items = parcelsRef.current.filter((parcel) => parcel.tier === cfg.tier);
      ...
      items.forEach((parcel, index) => {
        ...
        const glowPolygon = new maps.Polygon({ map: mapObj.current, paths, geodesic: false, strokeColor: glow, strokeOpacity: sold ? 0.72 : 0.18, strokeWeight: sold ? 6 : 4.5, fillColor: base, fillOpacity: sold ? 0.08 : 0.008, zIndex: sold ? 500 : 20, clickable: false });
        const mainPolygon = new maps.Polygon({ map: mapObj.current, paths, geodesic: false, strokeColor: core, strokeOpacity: sold ? 1 : 0.78, strokeWeight: sold ? 2.5 : 1.1, fillColor: base, fillOpacity: sold ? 0.08 : 0.008, zIndex: sold ? 501 : 21, clickable: true });
        const click = () => {
        ...
        const setHover = (value: boolean) => {
          ...
          const selected = multiSelectRef.current ? selectedIdsRef.current.has(parcel.id) : selectedRef.current === parcel.id;
        ...
        cell.path.forEach((point) => {
          ...
          cornerKeys.add(key);
          corners.current.set(`${parcel.id}:${key}`, new maps.Marker({
            map: mapObj.current,
            position: point,
  ...
  useEffect(() => {
    ...
    if (signature !== renderedGeometrySignatureRef.current) rebuildGeometry(maps);
      ...
      parcels.forEach((parcel) => {
        const current = parcelByIdRef.current.get(parcel.id);
        if (!current) return;
        const mode = multiSelectRef.current
          ? selectedIdsRef.current.has(parcel.id) ? "selected" : hoveredRef.current.has(parcel.id) ? "hover" : "normal"
          : selectedRef.current === parcel.id ? "selected" : hoveredRef.current.has(parcel.id) ? "hover" : "normal";
        applyParcelStyle(parcel.id, mode);
      });
    }
  }, [parcels, ready, center.lat, center.lng, rebuildGeometry, applyParcelStyle]);
  ...
  useEffect(() => {
    ...
    if (multiSelect) {
      previousMulti.forEach((id) => { if (!currentMulti.has(id)) applyParcelStyle(id, "normal"); });
      currentMulti.forEach((id) => applyParcelStyle(id, "selected"));
    }
    if (previousSelected && previousSelected !== selectedId && !multiSelect) applyParcelStyle(previousSelected, "normal");
    if (selectedId && !multiSelect) applyParcelStyle(selectedId, "selected");
    if (!multiSelect && previousMulti.size) previousMulti.forEach((id) => applyParcelStyle(id, "normal"));
