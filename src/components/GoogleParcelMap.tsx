...
function domeCell(center: CityCenter, index: number, total: number, innerRadius: number, outerRadius: number) {
  ...
  const a0 = (col / cols) * Math.PI * 2 - Math.PI / 2; const a1 = ((col + 1) / cols) * Math.PI * 2 - Math.PI / 2; const r0 = innerRadius + (row / rows) * (outerRadius - innerRadius); const r1 = innerRadius + ((row + 1) / rows) * (outerRadius - innerRadius);
  const project = (angle: number, radius: number) => { const x = Math.cos(angle) * radius; const y = Math.sin(angle) * radius; const radial = Math.min(1, radius / outerRadius); const dome = Math.sqrt(Math.max(0, 1 - radial * radial)); const lift = (1 - dome) * 0.01; const cos = Math.max(Math.cos(center.lat * Math.PI / 180), 0.2); return { lat: center.lat + y + lift, lng: center.lng + x / cos }; };
...
export function GoogleParcelMap({ parcels, selectedId, selectedIds = new Set<string>(), multiSelect = false, onSelect, onToggleSelect, onViewportChange, center }: Props) {
  ...
  useEffect(() => { if (!ready || !mapObj.current) return; const maps = (window as any).google.maps; cells.current.forEach((items) => items.forEach((polygon) => polygon.setMap(null))); cells.current.clear(); corners.current.forEach((marker) => marker.setMap(null)); corners.current.clear();
    ...
    configs.forEach((cfg) => { const items = parcels.filter((parcel) => parcel.tier === cfg.tier); items.forEach((parcel, index) => { const cell = domeCell(center, index, items.length, cfg.innerRadius, cfg.outerRadius); const paths = cell.path.map((point) => new maps.LatLng(point.lat, point.lng)); const sold = parcel.status === "sold"; const base = sold ? statusColor(parcel.status) : tierColor(parcel.tier); const line = sold ? base : "#e8c66a";
      const glow = new maps.Polygon({ map: mapObj.current, paths, geodesic: false, strokeColor: line, strokeOpacity: sold ? 0.95 : 0.20, strokeWeight: sold ? 7 : 4, fillColor: base, fillOpacity: sold ? 0.16 : 0.025, zIndex: sold ? 500 : 20, clickable: true }); const main = new maps.Polygon({ map: mapObj.current, paths, geodesic: false, strokeColor: line, strokeOpacity: sold ? 1 : 0.68, strokeWeight: sold ? 3 : 1, fillColor: base, fillOpacity: sold ? 0.08 : 0.015, zIndex: sold ? 501 : 21, clickable: true });
      const click = () => multiSelect ? (parcel.status === "available" ? toggleRef.current?.(parcel.id) : undefined) : selectRef.current(parcel.id); glow.addListener("click", click); main.addListener("click", click); cells.current.set(parcel.id, [glow, main]);
      cell.path.forEach((point) => { const key = `${point.lat.toFixed(8)}:${point.lng.toFixed(8)}`; if (cornerKeys.has(key)) return; cornerKeys.add(key); corners.current.set(`${parcel.id}:${key}`, new maps.Marker({ map: mapObj.current, position: point, icon: cornerIcon(maps, base, false), clickable: false, zIndex: sold ? 2000 : 1000, optimized: true })); });
    }); });
  ...
  useEffect(() => { if (!ready || !mapObj.current) return; const previousSelected = selectedRef.current; const previousMulti = previousSelectedIdsRef.current; const currentMulti = new Set(selectedIds);
    const restoreParcel = (id: string) => { const parcel = parcelsRef.current.find((item) => item.id === id); const parcelCells = cells.current.get(id); if (!parcel || !parcelCells) return; const color = parcel.status === "sold" ? statusColor(parcel.status) : tierColor(parcel.tier); const line = parcel.status === "sold" ? color : "#e8c66a"; parcelCells.forEach((polygon, index) => polygon.setOptions({ strokeOpacity: parcel.status === "sold" ? (index === 0 ? 0.95 : 1) : (index === 0 ? 0.20 : 0.68), strokeWeight: parcel.status === "sold" ? (index === 0 ? 7 : 3) : (index === 0 ? 4 : 1), fillOpacity: parcel.status === "sold" ? (index === 0 ? 0.16 : 0.08) : (index === 0 ? 0.025 : 0.015), zIndex: parcel.status === "sold" ? (index === 0 ? 500 : 501) : (index === 0 ? 20 : 21), strokeColor: line, fillColor: color })); };
    if (multiSelect) { previousMulti.forEach((id) => { if (!currentMulti.has(id)) restoreParcel(id); }); currentMulti.forEach((id) => { const parcel = parcelsRef.current.find((item) => item.id === id); const selectedCells = cells.current.get(id); if (!parcel || !selectedCells) return; const color = parcel.status === "sold" ? statusColor(parcel.status) : tierColor(parcel.tier); const line = parcel.status === "sold" ? color : "#ffd978"; selectedCells.forEach((polygon, index) => polygon.setOptions({ strokeOpacity: index === 0 ? 0.63 : 1, strokeWeight: index === 0 ? 8 : 2.2, fillOpacity: index === 0 ? 0.18 : 0.24, zIndex: index === 0 ? 300 : 301, strokeColor: line, fillColor: color })); }); }
    if (previousSelected && previousSelected !== selectedId && !multiSelect) restoreParcel(previousSelected);
