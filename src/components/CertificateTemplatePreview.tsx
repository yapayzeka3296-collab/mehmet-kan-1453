...
export function CertificateTemplatePreview({ tier, className = "" }: CertificateTemplatePreviewProps) {
  ...
  return <>
    ...
    {open && src && <div role="dialog" aria-modal="true" aria-label={`${LABELS[tier]} sertifika şablonu büyük önizleme`} className="fixed inset-0 z-[320] flex items-center justify-center bg-black/85 p-3 backdrop-blur-sm sm:p-6" onClick={() => setOpen(false)}><div className="relative flex max-h-[95vh] w-full max-w-[1400px] items-center justify-center" onClick={(event) => event.stopPropagation()}><button type="button" onClick={() => setOpen(false)} aria-label="Büyük önizlemeyi kapat" className="absolute right-2 top-2 z-[330] flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-black/80 text-2xl leading-none text-white shadow-xl transition hover:bg-black/95 focus:outline-none focus:ring-2 focus:ring-gold/70">×</button><img src={src} alt={`${LABELS[tier]} MySkyParcel sertifika şablonu büyük önizleme`} width={1122} height={794} className="max-h-[92vh] w-auto max-w-full rounded-md object-contain shadow-2xl" /></div></div>}
  </>;
}
