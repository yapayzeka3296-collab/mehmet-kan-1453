...
function cartItem(p: Parcel): ParcelCartItem | undefined {
  const tier = p.tier;
  if (tier !== "digital" && tier !== "elite" && tier !== "premium") return;
...
function SciFiGrid() {
  return <div aria-hidden className="pointer-events-none absolute inset-0 z-[3] overflow-hidden">
    <div className="absolute inset-[-25%] opacity-75" style={{ backgroundImage: "linear-gradient(rgba(76,224,255,.34) 1px,transparent 1px),linear-gradient(90deg,rgba(76,224,255,.34) 1px,transparent 1px),linear-gradient(rgba(177,108,255,.13) 1px,transparent 1px),linear-gradient(90deg,rgba(177,108,255,.13) 1px,transparent 1px)", backgroundSize: "32px 32px,32px 32px,160px 160px,160px 160px", transform: "perspective(680px) rotateX(54deg) scale(1.55) translateY(12%)", transformOrigin: "50% 57%", maskImage: "linear-gradient(to bottom,transparent 0%,black 25%,black 85%,transparent 100%)" }} />
    <div className="absolute inset-0" style={{ background: "radial-gradient(circle at 50% 52%,transparent 0 25%,rgba(0,15,30,.12) 48%,rgba(0,3,12,.65) 100%)" }} />
    <div className="absolute left-[7%] right-[7%] top-[19%] border-t border-cyan-200/25" />
    <div className="absolute left-[12%] right-[12%] top-[42%] border-t border-cyan-200/20" />
    <div className="absolute left-[17%] right-[17%] bottom-[23%] border-t border-cyan-200/15" />
    {[14, 31, 50, 69, 86].map((x, i) => <span key={i} className="absolute top-[19%] h-2 w-2 rounded-full bg-cyan-100 shadow-[0_0_7px_#55c9ff,0_0_18px_#55c9ff]" style={{ left: `${x}%` }} />)}
    {[22, 43, 65, 81].map((x, i) => <span key={i} className="absolute bottom-[23%] h-1.5 w-1.5 rounded-full bg-purple-200 shadow-[0_0_7px_#b77cff,0_0_16px_#b77cff]" style={{ left: `${x}%` }} />)}
  </div>;
}
...
export function SkyParcelMap({ parcels, selectedId, selectedIds = new Set(), multiSelect = false, onSelect, onToggleSelect, onViewportChange, center, focusTarget }: Props) {
  ...
  const groups = useMemo(() => {
    const g: Record<Tier, Parcel[]> = { digital: [], elite: [], premium: [] };
    parcels.forEach(p => { if (p.tier in g) g[p.tier as Tier].push(p); });
    return g;
  }, [parcels]);
  ...
  useEffect(() => {
    ...
    setFocus(true);
    const t = window.setTimeout(() => setFocus(false), 6000);
  ...
  const click = (p: Parcel) => {
    ...
    if (multiSelect && item) {
      const c = readParcelCart();
      const next = c.some(x => x.id === p.id) ? c.filter(x => x.id !== p.id) : [...c, item];
