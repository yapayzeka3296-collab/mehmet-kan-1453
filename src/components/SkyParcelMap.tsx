...
export function SkyParcelMap({parcels,selectedId,selectedIds=new Set(),multiSelect=false,onSelect,onToggleSelect,onViewportChange,center,focusTarget}:Props){
 ...
 return <div className="relative h-[350px] w-full overflow-hidden rounded-2xl border border-cyan-300/20 bg-[#071a2d] sm:h-[420px] lg:h-[469px]">
  ...
  <div className="absolute inset-0 z-[1] flex items-center justify-center overflow-hidden px-3 py-4 sm:px-6">
   <img src="/images/cities/turkey-3d-map.png" alt="Türkiye 3D haritası" className={`h-full w-full object-contain drop-shadow-[0_0_35px_rgba(44,190,255,.22)] transition-transform duration-[1400ms] ease-out ${focus?"scale-[1.08]":"scale-100"}`} />
  </div>
...
// Turkey 3D map image is used only as the visual base; parcel data, selection, filters and cart logic remain unchanged.