...
export function SkyParcelMap({ parcels, selectedId, selectedIds = new Set<string>(), multiSelect = false, onSelect, onToggleSelect, onViewportChange, center, focusTarget }: Props) {
  ...
  const renderParcel = (p: Parcel) => {
    ...
    if (polygonMode && hasUsableGeometry(p)) {
      ...
      return <path key={p.id} d={d} fill={selected ? "rgba(255,211,92,.42)" : `${color}20`} stroke={selected ? "#fff4b0" : `${color}0D`} strokeWidth={selected ? 0.0275 : 0.01375} vectorEffect="non-scaling-stroke" fillRule="evenodd" className="cursor-pointer transition-[fill,stroke] duration-150" style={{ opacity: p.status === "sold" ? 0.82 : 1 }} onClick={() => click(p)} onMouseEnter={() => setHover(p.id)} onMouseLeave={() => setHover(null)} aria-label={p.parcel_number} />;
    }
    const xy = geoPos(p); if (!xy) return null;
    return <button key={p.id} type="button" disabled={p.status !== "available"} aria-label={p.parcel_number} onClick={() => click(p)} onMouseEnter={() => setHover(p.id)} onMouseLeave={() => setHover(null)} className="absolute z-10 h-6 w-6 -translate-x-1/2 -translate-y-1/2 cursor-pointer rounded-[3px] border p-0 sm:h-7 sm:w-7" style={{ left: `${xy.left}%`, top: `${xy.top}%`, borderColor: selected ? "#fff4b0" : color, background: selected ? "rgba(255,211,92,.25)" : "rgba(4,18,30,.8)", boxShadow: selected ? "0 0 8px #fff4b0,0 0 22px rgba(255,211,92,.8)" : `0 0 5px ${color}0D`, opacity: p.status === "sold" ? 0.82 : 1 }}><span className="absolute inset-[5px] rounded-full" style={{ background: color }} />{(hover === p.id || selected) && <span className="pointer-events-none absolute left-1/2 top-[-30px] -translate-x-1/2 whitespace-nowrap rounded bg-[#020914]/95 px-2 py-1 text-[9px] text-white shadow-lg">{p.parcel_number}</span>}</button>;
  };
