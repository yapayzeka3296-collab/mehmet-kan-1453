...
export function SkyParcelMap({ parcels, selectedId, selectedIds = new Set<string>(), multiSelect = false, onSelect, onToggleSelect, onViewportChange, center, focusTarget }: Props) {
  ...
  const renderParcel = (p: Parcel) => {
    ...
    if (polygonMode && hasUsableGeometry(p)) {
      ...
      return <path key={p.id} d={d} fill={selected ? "rgba(255,211,92,.42)" : `${color}20`} stroke={selected ? "#fff4b0" : color} strokeWidth={selected ? 0.22 : 0.11} vectorEffect="non-scaling-stroke" fillRule="evenodd" className="cursor-pointer transition-[fill,stroke] duration-150" style={{ opacity: p.status === "sold" ? 0.82 : 1 }} onClick={() => click(p)} onMouseEnter={() => setHover(p.id)} onMouseLeave={() => setHover(null)} aria-label={p.parcel_number} />;
    }
