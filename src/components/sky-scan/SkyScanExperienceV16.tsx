...
export function SkyScanExperienceV16() {
  ...
  const loadParcels = useCallback(async (pos: Gps) => {
    try {
      ...
      const { data, error: qError } = await supabaseBrowser.from("sky_scan_parcels")
        ...
        .gte("longitude", pos.longitude - dLon).lte("longitude", pos.longitude + dLon).limit(500);
      ...
      setParcels(rows);
      setMessage(rows.length ? `${rows.length} parsel hazır · 4 m yürüyün` : "5 km içinde parsel bulunamadı");
  ...
  const updateGps = useCallback((position: GeolocationPosition) => {
    ...
    if (!originRef.current) { originRef.current = next; void loadParcels(next); }
    if (previous && dist(previous, next) >= 4 && calibrationSensorRef.current === null && sensorRef.current !== null) {
      calibrationBearingRef.current = bearing(previous, next);
      calibrationSensorRef.current = sensorRef.current;
      setAligned(true);
      setMessage("AR hizalandı · kamerayı çevirdiğiniz yönde parselleri gösteriyorum");
    }
  ...
  const startCameraAR = useCallback(async () => {
    ...
    try {
      ...
      setStarted(true);
      setMessage(sensorReady ? "Kamera AR açık · 4 m yürüyün" : "Kamera AR açık · sensör bekleniyor, 4 m yürüyün");
    } catch (e) {
  ...
  const projected = useMemo(() => {
    if (!started || !aligned || heading === null) return [];
    const fov = 65;
  ...
  }, [started, aligned, heading, parcels]);
