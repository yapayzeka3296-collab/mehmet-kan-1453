...
const directionFromRelative = (a: number) => Math.abs(a) < 20 ? "Karşında" : Math.abs(a) > 160 ? "Arkanda" : a > 0 ? "Sağında" : "Solunda";
...
export function SkyScanExperienceV16() {
  ...
  const updateGps = useCallback((position: GeolocationPosition) => {
    ...
    if (!previous || moved >= MIN_MOVE || Date.now() - lastFetchRef.current > 15000) void loadParcels(next);

    if (previous && moved >= MIN_MOVE && started && !aligned && lastPoseRef.current && originGpsRef.current) {
      ...
      if (geoRootRef.current) geoRootRef.current.rotation.y = xrYaw - rad(gpsBearing);
      setHeading(gpsBearing);
      setAligned(true);
      setMessage("Gerçek WebXR AR hizalandı · parseller gerçek 3B dünyaya bağlandı");
    }
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      if (!xr) {
        if (active) { setSupported(false); setMessage("Bu tarayıcıda WebXR bulunmuyor."); }
        return;
      }
      try {
        ...
        if (active) {
          setSupported(ok);
          setMessage(ok ? "Gerçek WebXR AR hazır" : "Bu cihaz/tarayıcı immersive-ar desteklemiyor.");
        }
      } catch {
        if (active) { setSupported(false); setMessage("WebXR immersive-ar desteği doğrulanamadı."); }
      }
    };
    void check();
    return () => { active = false; };
  ...
  const startXR = useCallback(async () => {
    setError(null);
    const xr = (navigator as XRNavigator).xr;
    if (!xr) { setError("navigator.xr yok. Bu tarayıcı gerçek WebXR AR desteklemiyor."); return; }

    try {
      const ok = await xr.isSessionSupported("immersive-ar");
      if (!ok) throw new Error("Bu cihaz/tarayıcı immersive-ar desteklemiyor. WebXR AR oturumu başlatılamaz.");

      // Kritik düzeltme: Daha önce NotSupportedError üreten session configuration kaldırıldı.
      // İlk oturum tamamen varsayılan özelliklerle açılıyor. local/hit-test/anchors zorunlu tutulmuyor.
      const session = await xr.requestSession("immersive-ar");
      sessionRef.current = session;

      const renderer = rendererRef.current;
      if (!renderer) throw new Error("WebXR renderer hazır değil.");
      await renderer.xr.setSession(session);

      // immersive session için local en düşük ortak stabil dünya uzayıdır.
      ...
      setHeading(null);
      setStarted(true);
      setMessage(gps ? "Gerçek AR açık · 4 m yürüyün, GPS yönü XR dünyasına kalibre edilecek" : "Gerçek AR açık · GPS bekleniyor");
      ...
      const name = e instanceof DOMException ? e.name : e instanceof Error ? e.name : "Error";
      const text = e instanceof Error ? e.message : String(e);
      setError(`${name}: ${text}`);
    }
  }, [gps]);

  const stopXR = useCallback(() => { void sessionRef.current?.end(); }, []);
  ...
  useEffect(() => {
    ...
    renderer.setAnimationLoop((_time, frame) => {
      if (frame && spaceRef.current) lastPoseRef.current = frame.getViewerPose(spaceRef.current);
  ...
  useEffect(() => {
    ...
    root.clear();
    for (const parcel of parcels.slice(0, 100)) {
      ...
      const marker = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: 0x22d3ee, transparent: true, opacity: 0.82 })
      );
  ...
  const nearestDirection = useMemo(() => nearest && heading !== null ? directionFromRelative(signed(nearest.bearing - heading)) : "—", [heading, nearest]);
  ...
  return <div ref={host} className="relative h-[100dvh] w-full overflow-hidden bg-black text-white">
    ...
    {started && nearest && <div className="pointer-events-none absolute bottom-3 left-1/2 z-20 w-[calc(100%-20px)] max-w-sm -translate-x-1/2 rounded-xl bg-black/75 px-3 py-2 backdrop-blur">
      ...
      <div className="mt-0.5 flex items-center justify-between text-[10px] text-white/75"><span>{aligned ? "Gerçek 3B XR konumuna bağlı" : "4 m yürüyerek yönü kalibre et"}</span><span><Navigation className="mr-0.5 inline h-3 w-3" />{nearestDirection}</span></div>
    </div>}

    {!started && <div className="absolute bottom-3 left-1/2 z-20 w-[calc(100%-20px)] max-w-sm -translate-x-1/2 rounded-xl bg-black/80 p-3 backdrop-blur">
      <div className="mb-1 flex items-center gap-2 text-sm font-semibold"><LocateFixed className="h-4 w-4" />Gerçek WebXR AR</div>
      <div className="mb-2 text-[10px] leading-4 text-white/75">Kamera WebXR tarafından gerçek AR oturumu olarak yönetilir. Parseller Three.js 3B nesneleri olarak XR dünya koordinatına yerleştirilir.</div>
      {error && <div className="mb-2 rounded-lg bg-red-950/70 p-2 text-[10px] leading-4 text-red-200">{error}</div>}
      <button disabled={supported !== true} onClick={() => void startXR()} className="flex w-full items-center justify-center gap-2 rounded-lg bg-white px-3 py-2.5 text-xs font-bold text-black disabled:cursor-not-allowed disabled:opacity-40">
        {supported === true ? "Gerçek AR'yi başlat" : supported === false ? "Bu cihazda WebXR AR yok" : "WebXR kontrol ediliyor…"}
      </button>
    </div>}
  </div>;
}
