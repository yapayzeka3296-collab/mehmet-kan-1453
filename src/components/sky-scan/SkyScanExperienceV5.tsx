...
export function SkyScanExperienceV5() {
    ...
    const animate = () => {
      ...
      if (visibleTickRef.current++ % 5 === 0) {
        ...
        const visible = projected.filter((parcel) => {
          ...
          return point.z >= -1 && point.z <= 1 && point.x >= -1.12 && point.x <= 1.12 && point.y >= -1.12 && point.y <= 1.12;
        });
  ...
  useEffect(() => {
    ...
    for (const key of directionKeys) {
      const parcel = candidates.find((item) => directionFromBearing(item.bearing, heading) === key);
      if (parcel) { selectedForRender.push(parcel); used.add(parcel.id); }
    }
    for (const parcel of candidates) {
      if (selectedForRender.length >= MAX_VISIBLE) break;
      if (!used.has(parcel.id)) { selectedForRender.push(parcel); used.add(parcel.id); }
    }
    const scene = sceneRef.current;
    const nextIds = new Set(selectedForRender.map((parcel) => parcel.id));
    for (const [id, group] of parcelGroupsRef.current) {
      if (!nextIds.has(id)) {
        ...
        parcelGroupsRef.current.delete(id);
      }
    }
    for (const parcel of selectedForRender) {
