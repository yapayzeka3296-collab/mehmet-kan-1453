...
export function SkyScanExperienceV10() {
  ...
  const load = useCallback(async (pos: Gps) => {
    ...
    try {
      ...
      const q = await Promise.race([query, new Promise<never>((_, reject) => window.setTimeout(() => reject(new Error("Parsel sorgusu zaman aşımına uğradı")), 12000))]);
      ...
      const rows = (q.data ?? []).map((r: any) => {
        ...
        const distance = hav(pos, target), b = bearing(pos, target), raw = enu(pos, [target.longitude, target.latitude]);
  ...
  const disposeScene = useCallback(() => {
    ...
    cancelAnimationFrame(current.raf); window.removeEventListener("resize", current.resize); current.renderer.dispose(); sceneRef.current = null;
  ...
  const addParcels = useCallback((root: THREE.Group, labels: THREE.Group, pos: Gps, items: Parcel[]) => {
    ...
    items.forEach((p, index) => {
      ...
      if (p.polygon.length >= 3) {
        ...
        p.polygon.forEach((pt, i) => { const v = enu(pos, pt), scale = p.distance > 0 ? compressedDistance(p.distance) / p.distance : 1, x = v.x * scale, z = v.z * scale; if (i) shape.lineTo(x, z); else shape.moveTo(x, z); });
        shape.closePath();
        const geo = new THREE.ExtrudeGeometry(shape, { depth: 8, bevelEnabled: false }); geo.rotateX(-Math.PI / 2); geo.translate(0, target.y, 0);
      ...
      const marker = new THREE.Mesh(new THREE.SphereGeometry(Math.max(2.5, Math.min(7, 120 / Math.max(p.distance, 30))), 16, 16), new THREE.MeshBasicMaterial({ color: 0xffffff, depthTest: false })); marker.position.copy(target); root.add(marker);
      const c = document.createElement("canvas"); c.width = 520; c.height = 100; const ctx = c.getContext("2d")!; ctx.fillStyle = "rgba(0,0,0,.82)"; ctx.fillRect(4, 4, 512, 92); ctx.fillStyle = "#fff"; ctx.font = "bold 30px sans-serif"; ctx.fillText(p.parcel_number, 18, 42); ctx.font = "22px sans-serif"; ctx.fillText(`${Math.round(p.distance)} m · ${Math.round(p.bearing)}°`, 18, 75);
      const texture = new THREE.CanvasTexture(c); const sprite = new THREE.Sprite(new THREE.SpriteMaterial({ map: texture, transparent: true, depthTest: false })); sprite.position.set(target.x, target.y + 10, target.z); sprite.scale.set(18, 3.5, 1); sprite.userData.baseY = target.y + 10; sprite.userData.phase = index * 0.37; labels.add(sprite);
    });
  }, []);
  ...
  const draw = useCallback((pos: Gps, items: Parcel[]) => {
    ...
    disposeScene();
    const scene = new THREE.Scene(), cam = new THREE.PerspectiveCamera(70, el.clientWidth / Math.max(el.clientHeight, 1), 0.1, 50000);
    ...
    const euler = new THREE.Euler(), sensorQ = new THREE.Quaternion(), smoothQ = new THREE.Quaternion(); let lastDebug = 0;
    const tick = (now: number) => {
      const current = oRef.current;
      if (current.alpha !== null && current.beta !== null && current.gamma !== null) {
        euler.set(rad(current.beta), rad(current.alpha), rad(-current.gamma), "YXZ"); sensorQ.setFromEuler(euler); sensorQ.multiply(q1); sensorQ.multiply(q0.setFromAxisAngle(zee, -rad(window.screen.orientation?.angle ?? 0))); smoothQ.slerp(sensorQ, smoothQ.lengthSq() === 0 ? 1 : 0.2); cam.quaternion.copy(smoothQ);
      } else if (current.heading !== null) {
        const yaw = -rad(norm(current.heading + (window.screen.orientation?.angle ?? 0))); euler.set(rad(-current.pitch), yaw, rad(current.roll), "YXZ"); sensorQ.setFromEuler(euler); smoothQ.slerp(sensorQ, smoothQ.lengthSq() === 0 ? 1 : 0.2); cam.quaternion.copy(smoothQ);
      }
      ...
      if (items[0] && debugOpen && now - lastDebug > 200) {
        const forward = new THREE.Vector3(0, 0, -1).applyQuaternion(cam.quaternion), cameraBearing = norm(Math.atan2(forward.x, -forward.z) * 180 / Math.PI), rel = oRef.current.heading === null ? null : angleDelta(items[0].bearing, oRef.current.heading), projected = items[0].world.clone().project(cam);
        setDebug({ target: items[0].world.clone(), inFrustum: projected.z >= -1 && projected.z <= 1 && Math.abs(projected.x) <= 1 && Math.abs(projected.y) <= 1, relativeBearing: rel, cameraBearing }); lastDebug = now;
      }
      renderer.render(scene, cam); const raf = requestAnimationFrame(tick); if (sceneRef.current) sceneRef.current.raf = raf;
    };
  ...
  const start = useCallback(async () => {
    sensorCleanup.current?.(); if (sensorTimer.current !== null) window.clearTimeout(sensorTimer.current);
    setStarted(true); setError(null); setSensor("Sensör başlatılıyor…");
    const granted = await permissions(); if (!granted) setSensor("Sensör izni verilmedi · kamera ve GPS devam ediyor");
    ...
    const onOrientation = (e: Event) => {
      ...
      seenOrientation = true; if (r.absolute) absoluteLocked = true;
      const previous = oRef.current;
      const merged: Orientation = { ...r, heading: r.heading ?? previous.heading, absolute: r.heading === null ? previous.absolute : r.absolute, source: r.heading === null && previous.heading !== null ? previous.source : r.source };
      if (!absoluteLocked || r.absolute || r.source === "iOS pusula" || r.heading !== null) { oRef.current = merged; setO(merged); }
      setSensor(merged.heading === null ? "Hareket sensörü aktif · pusula aranıyor" : merged.absolute ? "Mutlak pusula aktif" : "Pusula aktif");
    };
    ...
    if (navigator.geolocation) {
      watch.current = navigator.geolocation.watchPosition((p) => {
        ...
        setGps(n); const old = lastLoad.current, improved = old && (n.accuracy ?? 999) + 10 < (old.accuracy ?? 999); if (!old || hav(old, n) > 100 || improved) void load(n);
        if (oRef.current.heading === null && n.heading !== null && (n.speed ?? 0) >= 1) setSensor("GPS hareket yönü aktif · pusula sensörü bekleniyor");
    ...
    try {
      const s = await navigator.mediaDevices.getUserMedia({ video: { facingMode: { ideal: "environment" }, width: { ideal: 1920 }, height: { ideal: 1080 } }, audio: false }); stream.current = s; if (video.current) { video.current.srcObject = s; await video.current.play(); }
    } catch (e) { setError(e instanceof Error ? e.message : "Kamera açılamadı"); }
  }, [load]);

  const stop = useCallback(() => {
    sensorCleanup.current?.(); sensorCleanup.current = null; if (sensorTimer.current !== null) { window.clearTimeout(sensorTimer.current); sensorTimer.current = null; }
    if (watch.current !== null) { navigator.geolocation.clearWatch(watch.current); watch.current = null; }
    stream.current?.getTracks().forEach((t) => t.stop()); stream.current = null; disposeScene(); setStarted(false);
  }, [disposeScene]);

  useEffect(() => () => stop(), [stop]);

  const nearest = parcels[0];
  const gpsHeading = gps?.heading !== null && (gps?.speed ?? 0) >= 1 ? gps.heading : null;
  const effectiveHeading = o.heading ?? gpsHeading;
  const dir = nearest && effectiveHeading !== null ? angleDelta(nearest.bearing, effectiveHeading) : null;
  const dirText = dir === null ? "Yön bekleniyor" : Math.abs(dir) < 25 ? "DÜZ" : dir > 0 && dir < 160 ? "SAĞ" : dir < 0 && dir > -160 ? "SOL" : "ARKA";
  const label = o.heading !== null ? (o.absolute ? "Mutlak pusula aktif" : "Pusula aktif") : gpsHeading !== null ? "GPS hareket yönü aktif" : sensor;
  const arrowText = dir === null ? "Telefonu hareket ettirin / pusula bekleniyor…" : Math.abs(dir) <= 35 ? `↑ ${Math.round(Math.abs(dir))}°` : dir > 0 ? `→ ${Math.round(Math.abs(dir))}° Sağda` : `← ${Math.round(Math.abs(dir))}° Solda`;
  ...
  return <div className="relative h-[100dvh] w-full overflow-hidden bg-black text-white">
    ...
    <div className="absolute inset-x-0 top-0 z-20 p-3"><div className="mx-auto max-w-xl rounded-2xl bg-black/65 p-3">
      ...
      <div className="mt-2 grid grid-cols-3 gap-2 text-xs"><span>GPS: {gps ? `${Math.round(gps.accuracy ?? 0)} m` : "—"}</span><span>Heading: {effectiveHeading === null ? "—" : `${Math.round(effectiveHeading)}°`}</span><span>Parsel: {parcels.length}</span></div>
      ...
      {debugOpen && <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 rounded-xl bg-black/80 p-2 font-mono text-[10px] text-white/90"><span>Lat: {gps?.latitude.toFixed(6) ?? "—"}</span><span>Lng: {gps?.longitude.toFixed(6) ?? "—"}</span><span>Accuracy: {gps?.accuracy?.toFixed(1) ?? "—"}</span><span>Speed: {gps?.speed?.toFixed(1) ?? "—"}</span><span>Alpha: {o.alpha?.toFixed(1) ?? "—"}</span><span>Absolute: {String(o.absolute)}</span><span>Heading: {effectiveHeading?.toFixed(1) ?? "—"}</span><span>Beta: {o.beta?.toFixed(1) ?? "—"}</span><span>Gamma: {o.gamma?.toFixed(1) ?? "—"}</span><span>World X: {debug.target.x.toFixed(1)}</span><span>World Y: {debug.target.y.toFixed(1)}</span><span>World Z: {debug.target.z.toFixed(1)}</span><span>Pitch: {o.pitch.toFixed(1)}°</span><span>Camera bearing: {debug.cameraBearing?.toFixed(1) ?? "—"}°</span><span>Frustum: {debug.inFrustum ? "Evet" : "Hayır"}</span><span>Rel. bearing: {debug.relativeBearing?.toFixed(1) ?? "—"}°</span></div>}
    </div></div>
