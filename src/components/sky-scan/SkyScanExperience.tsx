import { Camera, Compass, LocateFixed, MapPin, Navigation, ShoppingCart, X } from "lucide-react";
...
function getScreenAngle() { return typeof screen !== "undefined" && screen.orientation ? Number(screen.orientation.angle) || 0 : 0; }
function headingFromEvent(event: OrientationEventWithCompass, absolute: boolean) {
  if (typeof event.webkitCompassHeading === "number" && Number.isFinite(event.webkitCompassHeading)) return event.webkitCompassHeading;
  if (absolute && typeof event.alpha === "number" && Number.isFinite(event.alpha)) return (360 - event.alpha + getScreenAngle() + 360) % 360;
  return null;
}
...
export function SkyScanExperience() {
  ...
  useEffect(() => {
    ...
    const onAbsolute = (event: DeviceOrientationEvent) => { const value = headingFromEvent(event as OrientationEventWithCompass, true); if (value === null) return; absoluteReceived = true; setHeading(value); setOrientationError(null); };
    const onRelative = (event: DeviceOrientationEvent) => { if (absoluteReceived) return; const value = headingFromEvent(event as OrientationEventWithCompass, false); if (value === null) return; setHeading(value); setOrientationError(null); };
    ...
    orientationTimer.current = window.setTimeout(() => { if (!absoluteReceived) setOrientationError("Pusula verisi alınamadı. Kamera açık kalacak; gerçek parseller aşağıdaki listeden seçilebilir."); }, 2500);
    return () => { window.removeEventListener("deviceorientationabsolute", onAbsolute as EventListener, true); window.removeEventListener("deviceorientation", onRelative as EventListener, true); if (orientationTimer.current !== null) window.clearTimeout(orientationTimer.current); };
  }, [orientationStarted]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      else { const rows = (data ?? []) as SkyParcel[]; setNearbyParcels(rows.map((parcel) => ({ ...parcel, distance: distanceMeters(location, parcel), bearing: bearingDegrees(location, parcel) })).filter((parcel) => parcel.distance <= MAX_DISTANCE_METERS).sort((a, b) => a.distance - b.distance)); }
  ...
  const startScan = async () => {
    setCameraError(null); setCameraReady(false); setOrientationError(null);
    const orientationGranted = await requestOrientationPermission();
    if (!orientationGranted) setOrientationError("Yön sensörü izni verilmedi. Kamera açıldı; parseller liste modunda gösterilecek.");
    try {
  ...
  const stopScan = () => { streamRef.current?.getTracks().forEach((track) => track.stop()); streamRef.current = null; if (videoRef.current) { videoRef.current.pause(); videoRef.current.srcObject = null; } setCameraStarted(false); setCameraReady(false); setOrientationStarted(false); setHeading(null); setSelectedParcel(null); };
