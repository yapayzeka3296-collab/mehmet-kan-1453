...
export function SkyScanExperience() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data, error } = await supabaseBrowser.rpc("sky_scan_parcels", {
        p_min_lat: location.latitude - 0.045,
        p_min_lng: location.longitude - 0.06,
        p_max_lat: location.latitude + 0.045,
        p_max_lng: location.longitude + 0.06,
        p_limit: 80,
      });
      if (!alive) return;
      if (error) {
        ...
        setNearbyParcels(rows.map((parcel) => ({
          ...parcel,
  ...
  const visibleParcels = useMemo(() => {
    ...
    return nearbyParcels
      ...
      .filter(({ parcel, angle }) => Math.abs(angle) <= CAMERA_FOV_DEGREES / 2 && parcel.distance <= MAX_DISTANCE_METERS)
  ...
  const buySelected = () => {
    ...
    const ids = encodeURIComponent(selectedParcel.id);
    window.location.href = `/parsel-satin-al?parcels=${ids}`;
  };
    ...
    <main className="min-h-screen bg-slate-950 text-white">
      <section className="relative mx-auto flex min-h-screen w-full max-w-6xl flex-col overflow-hidden px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="relative mt-6 min-h-[72vh] flex-1 overflow-hidden rounded-3xl border border-cyan-300/15 bg-slate-950 shadow-2xl">
          ...
          {cameraStarted ? <div className="absolute inset-0 overflow-hidden">
            {!cameraReady && <div className="absolute inset-0 z-40 flex items-center justify-center bg-black"><div className="rounded-2xl border border-cyan-300/20 bg-slate-950/90 px-6 py-5 text-center"><Camera className="mx-auto h-8 w-8 animate-pulse text-cyan-300" /><p className="mt-3 text-sm font-semibold">Kamera görüntüsü hazırlanıyor…</p></div></div>}
            <div className="pointer-events-none absolute inset-x-0 top-0 z-30 flex justify-center pt-4"><div className="rounded-full border border-white/20 bg-black/60 px-4 py-2 text-xs font-semibold backdrop-blur-md">🧭 {heading === null ? "Yön algılanıyor…" : `${heading.toFixed(0)}°`} · {visibleParcels.length} parsel</div></div>
            <div className="absolute inset-0 z-20">
              {visibleParcels.map(({ parcel, angle }) => {
                ...
                return <button key={parcel.id} type="button" onClick={() => setSelectedParcel(parcel)} className="absolute -translate-x-1/2 text-left transition-all duration-150 active:scale-95" style={{ left: `${leftPercent}%`, top: "38%" }} aria-label={`${parcel.parcel_number} parselini görüntüle`}>
                  <div className="flex flex-col items-center">
                    <div className={`relative flex h-16 w-16 items-center justify-center rounded-2xl border-2 ${selected ? "border-amber-200 bg-amber-300/30 shadow-[0_0_35px_rgba(251,191,36,.9)]" : "border-cyan-200/80 bg-cyan-400/20 shadow-[0_0_30px_rgba(34,211,238,.65)]"} backdrop-blur-sm`}>
                      <div className="absolute inset-2 rotate-45 rounded-lg border border-white/70" /><span className="relative text-[9px] font-black tracking-wider">SKY</span>
                    </div>
                    <div className="mt-2 whitespace-nowrap rounded-xl border border-white/20 bg-black/70 px-3 py-2 text-center shadow-xl backdrop-blur-md"><p className="text-xs font-bold text-cyan-100">✦ {parcel.parcel_number}</p><p className="mt-0.5 text-[10px] text-white/75">{formatDistance(parcel.distance)} · {parcel.bearing.toFixed(0)}°</p></div>
                  </div>
                </button>;
              })}
            </div>
...
function StatusCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) { return <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-4"><div className="text-cyan-300">{icon}</div><div className="min-w-0"><p className="text-[10px] uppercase tracking-wider text-white/40">{label}</p><p className="mt-1 truncate text-sm font-semibold">{value}</p></div></div>; }
