...
function Profilim() {
  ...
  async function updateProfile(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const { error: authError } = await supabaseBrowser.auth.updateUser({ data: { full_name: cleanName, phone: cleanPhone, city: cleanCity } });
      if (authError) throw authError;
      const { error: profileError } = await supabaseBrowser.from("profiles").update({ full_name: cleanName, phone: cleanPhone || null, city: cleanCity || null }).eq("id", user.id);
      ...
      <main className="mx-auto grid max-w-[1600px] gap-6 px-4 py-8 lg:grid-cols-[260px_1fr] lg:px-8">
        ...
        <div className="min-w-0">
          ...
          <form className="panel mt-6 grid gap-5 p-6 sm:grid-cols-2" onSubmit={updateProfile}>
            <label className="block"><span className="text-xs text-muted-foreground">Ad Soyad</span><input value={fullName} onChange={(e) => setFullName(e.target.value)} autoComplete="name" className="mt-1.5 w-full rounded-md border border-input bg-background/50 px-3 py-2.5 text-sm outline-none focus:border-gold" /></label>
            ...
            <label className="block"><span className="text-xs text-muted-foreground">Telefon</span><input value={phone} onChange={(e) => setPhone(e.target.value)} autoComplete="tel" className="mt-1.5 w-full rounded-md border border-input bg-background/50 px-3 py-2.5 text-sm outline-none focus:border-gold" /></label>
            <label className="block"><span className="text-xs text-muted-foreground">Şehir</span><input value={city} onChange={(e) => setCity(e.target.value)} autoComplete="address-level2" className="mt-1.5 w-full rounded-md border border-input bg-background/50 px-3 py-2.5 text-sm outline-none focus:border-gold" /></label>
