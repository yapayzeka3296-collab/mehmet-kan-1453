...
function Parsellerim() {
    ...
    async function load() {
      ...
      try {
        const { data, error: e } = await supabaseBrowser
          .from("parcels")
          .select(
            "id, parcel_number, status, owner_id, price, latitude, longitude, created_at, updated_at"
          )
          .eq("owner_id", user.id)
  ...
  const summaryCounts = () => {
    ...
    const gifted = 0; // depends on domain data
    const expired = parcels.filter((p) => {
      // crude: treat sold as expired for summary logic placeholder
      return p.status === "sold";
    }).length;

    return [
      ["Toplam Parsel", String(total)],
      ["Aktif Parsel", String(active)],
      ["Hediye Edilen", String(gifted)],
      ["Süresi Dolan", String(expired)],
    ];
  };
        ...
        <div className="min-w-0 grid gap-6 xl:grid-cols-[1fr_300px]">
          <div className="min-w-0">
            <div className="panel relative overflow-hidden p-6">
              ...
              <div className="relative">
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  Ana Sayfa <span className="mx-2">›</span> Kullanıcı Paneli <span className="mx-2">›</span>{" "}
                  <span className="text-gold">Parsellerim</span>
                </p>
              </div>
            </div>
                  ...
                  <li key={t.key}>
                    <button
                      className={`rounded-md px-4 py-2.5 text-[11px] tracking-[0.06em] ${
                        i === 0 ? "border border-gold/60 text-gold" : "text-muted-foreground hover:text-gold"
                      }`}
                    >
                      {t.label}
                    </button>
            ...
            <ul className="mt-4 grid gap-4">
              {loading && (
                <li className="panel p-4 text-center text-sm text-muted-foreground">Parseller yükleniyor...</li>
              )}

              {!loading && error && (
                <li className="panel p-4 text-center text-sm text-destructive">{error}</li>
              )}

              {!loading && !error && parcels.length === 0 && (
                <li className="panel p-4 text-center text-sm text-muted-foreground">Henüz satın aldığınız bir parsel yok.</li>
              )}
                ...
                <li key={p.id} className="panel grid gap-4 p-4 md:grid-cols-[280px_1fr]">
                  <div className="relative overflow-hidden rounded-lg">
                    ...
                    <span className="absolute left-3 top-3 rounded bg-success px-2 py-0.5 text-[10px] font-bold text-background">
                      {p.status === 'reserved' ? 'AKTİF' : p.status.toUpperCase()}
                    </span>
                  </div>
                  <div className="min-w-0">
                    <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
                      <h2 className="flex min-w-0 items-center gap-2 truncate font-display text-xl">
                        {p.parcel_number} <Star className="h-4 w-4 shrink-0 text-gold" />
                      </h2>
                      <div className="flex shrink-0 items-center gap-2">
                        <span className="rounded-full border border-success/40 px-3 py-1 text-[11px] text-success">
                          {p.status === 'reserved' ? 'Aktif' : p.status}
                        </span>
                        <MoreVertical className="h-4 w-4 text-muted-foreground" />
                      </div>
                    </div>
                    <p className="mt-2 text-sm text-muted-foreground">{`Parsel ID: ${p.id}`}</p>
                    <div className="mt-5 grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4">
                      <div className="flex min-w-0 flex-wrap gap-6 text-xs">
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <span>
                            <span className="block text-muted-foreground">Fiyat</span>
                            {p.price}
                          </span>
                        </div>
                      </div>
