...
function Parsellerim() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (e) throw e;
        if (!mounted) return;
        setParcels(((data ?? []) as any[]).map((p) => ({ ...p, city_name: p.cities?.name, city_code: p.cities?.code })) as Parcel[]);
      } catch (err) { console.error(err); if (mounted) setError("Parseller yüklenirken hata oluştu"); }
  ...
  const summaryCounts = [
    ...
    ["Hediye Edilen", 0], ["Süresi Dolan", 0],
  ];
      ...
      <main className="mx-auto grid max-w-[1600px] gap-6 px-4 py-8 lg:grid-cols-[260px_1fr] lg:px-8">
        ...
        <div className="min-w-0 grid gap-6 xl:grid-cols-[1fr_300px]">
          <div className="min-w-0">
            <div className="panel relative overflow-hidden p-6"><img src={heroCity} alt="" aria-hidden loading="lazy" width={1920} height={1088} className="absolute inset-y-0 right-0 hidden h-full w-1/2 object-cover opacity-40 md:block" /><div className="relative"><h1 className="font-display text-3xl font-bold">PARSELLERİM</h1><p className="mt-2 text-xs text-muted-foreground">Ana Sayfa <span className="mx-2">›</span> Kullanıcı Paneli <span className="mx-2">›</span> <span className="text-gold">Parsellerim</span></p></div></div>
