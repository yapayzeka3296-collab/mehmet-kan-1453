...
export const Route = createFileRoute("/odeme-sonuc")({ validateSearch: (search: Record<string, unknown>) => ({ intent: typeof search.intent === "string" ? search.intent : "" }), head: () => ({ meta: [{ title: "Sipariş Durumu — MySkyParcel" }] }), component: PaymentResult });
function PaymentResult() {
  const navigate = useNavigate({ from: "/odeme-sonuc" }); const { intent } = Route.useSearch();
  const [status, setStatus] = useState<"checking" | "paid" | "waiting" | "failed">("checking"); const [message, setMessage] = useState("Shopier ödemeniz doğrulanıyor...");
  useEffect(() => {
    ...
    const check = async () => {
      ...
      if (!token) { setStatus("failed"); setMessage("Oturumunuz bulunamadı. Lütfen giriş yaparak siparişlerinizi kontrol edin."); return; }
      const response = await fetch(`/api/shopier-checkout-status?intent=${encodeURIComponent(intent)}`, { headers: { authorization: `Bearer ${token}` }, cache: "no-store" });
      const result = await response.json().catch(() => ({})); if (cancelled) return;
      if (!response.ok) { setStatus("waiting"); setMessage("Ödeme sonucu doğrulanıyor..."); return; }
      if (result.status === "paid") { setStatus("paid"); setMessage("Ödemeniz doğrulandı. Parselleriniz hesabınıza tanımlandı ve sertifika süreci başlatıldı."); if (timer) clearInterval(timer); return; }
      if (["failed", "cancelled", "expired"].includes(result.status)) { setStatus("failed"); setMessage("Ödeme tamamlanamadı veya ödeme oturumu sona erdi."); if (timer) clearInterval(timer); return; }
