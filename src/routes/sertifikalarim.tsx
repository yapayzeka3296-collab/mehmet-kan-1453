...
function Sertifikalarim() {
  ...
  </div></main>{selected && <div className="fixed inset-0 z-[2000] overflow-y-auto bg-black/80 p-3 sm:p-6" role="dialog" aria-modal="true" aria-label="Sertifika önizleme"><div className="mx-auto max-w-[576px] py-4 sm:py-8"><div className="fixed right-4 top-4 z-[2010] sm:right-6 sm:top-6"><button type="button" onClick={() => setSelected(null)} className="rounded-full border border-border bg-background/95 p-2 shadow-lg hover:border-gold" aria-label="Önizlemeyi kapat"><X className="h-5 w-5" /></button></div><CertificateRenderer certificate={selected} /></div></div>}<TrustBar items={SECURITY_TRUST} /><SiteFooter /></div>;
}
