...
import { Award, Eye, RefreshCw, ShieldCheck, X } from "lucide-react";
...
export const Route = createFileRoute("/sertifikalarim")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "MySkyParcel sertifikalarınızı görüntüleyin ve doğrulayın." },
    ],
  }),
...
function Sertifikalarim() {
  ...
  const loadCertificates = useCallback(async () => {
    ...
    const { data, error: queryError } = await supabaseBrowser
      ...
      .select("id,parcel_id,tier,status,certificate_number,requested_at,issued_at,holder_name_snapshot,city_name_snapshot,certificate_fingerprint,verification_url,parcel:parcels(parcel_number)")
      .eq("user_id", user.id)
  ...
  return (
    <div className="starfield min-h-screen">
        ...
        <div className="min-w-0">
          <div className="panel flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-xs text-muted-foreground">MySkyParcel sertifikalarınız, doğrulama bilgileri ve güncel şablonlarıyla birlikte burada saklanır.</p>
            </div>
          ...
          {!loading && !error && certificates.length === 0 && (
            <div className="panel mt-6 p-8 text-center">
              ...
              <p className="mx-auto mt-2 max-w-lg text-sm text-muted-foreground">Bir parseliniz için sertifika kaydı oluşturulduğunda burada görünecektir.</p>
            </div>
          )}
          ...
          {!loading && !error && certificates.length > 0 && (
            <section className="mt-6">
              <div className="mb-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">Şablonlar DİJİTAL, ÖZEL ve PREMIUM olarak ayrılır. Yayınlanmış sertifikalar doğrulama ve yazdırma/PDF akışına sahiptir.</p>
              </div>
                  ...
                  return (
                    <li key={certificate.id} className="panel p-5">
                      ...
                      <dl className="mt-4 space-y-2 text-xs text-muted-foreground">
                        <div className="flex justify-between gap-4"><dt>Durum</dt><dd className="text-foreground">{STATUS_LABELS[certificate.status]}</dd></div>
                        <div className="flex justify-between gap-4"><dt>Sertifika No</dt><dd className="text-right text-foreground">{certificate.certificate_number || "—"}</dd></div>
                        <div className="flex justify-between gap-4"><dt>Düzenlenme</dt><dd className="text-foreground">{certificate.issued_at ? new Date(certificate.issued_at).toLocaleDateString("tr-TR") : "—"}</dd></div>
                      </dl>
                      <div className="mt-5 flex flex-wrap gap-2">
                        <button type="button" disabled={!issued} onClick={() => setSelected(certificate)} className="inline-flex items-center gap-2 rounded-md border border-input px-3 py-2 text-xs hover:bg-accent disabled:cursor-not-allowed disabled:opacity-40">
                          <Eye className="h-3.5 w-3.5" /> Görüntüle
                        </button>
                        {issued && certificate.certificate_number && (
                          <a href={`/sertifika-dogrula?code=${encodeURIComponent(certificate.certificate_number)}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-md border border-gold/40 px-3 py-2 text-xs text-gold hover:bg-gold/10">
      ...
      {selected && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/80 p-3 sm:p-6" role="dialog" aria-modal="true" aria-label="Sertifika önizleme">
          <div className="mx-auto max-w-6xl py-4 sm:py-8">
            <div className="mb-3 flex justify-end">
              <button type="button" onClick={() => setSelected(null)} className="rounded-full border border-border bg-background/90 p-2 hover:border-gold" aria-label="Önizlemeyi kapat">
                <X className="h-5 w-5" />
              </button>
            </div>
            <CertificateRenderer certificate={selected} />
          </div>
        </div>
      )}
