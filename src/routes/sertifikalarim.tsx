...
function CertificateModal({ certificate, onClose }: { certificate: Certificate; onClose: () => void }) {
  ...
  return createPortal(
    ...
    >
      <div className="relative mx-auto w-full max-w-[576px] py-4 sm:py-8">
        <div className="absolute right-0 top-0 z-[10010] sm:-right-2 sm:top-0">
          <button type="button" onClick={onClose} className="rounded-full border border-border bg-background/95 p-2 shadow-lg hover:border-gold" aria-label="Önizlemeyi kapat">
            <X className="h-5 w-5" />
          </button>
        </div>
...
function Sertifikalarim() {
  ...
  return <div className="starfield min-h-screen"><SiteHeader /><main className="mx-auto grid max-w-[1600px] gap-6 px-4 py-8 lg:grid-cols-[260px_1fr] lg:px-8"><UserSidebar active="/sertifikalarim" /><div className="min-w-0">
    ...
    {!loading && !error && certificates.length === 0 && <div className="panel mt-6 p-8 text-center"><Award className="mx-auto h-12 w-12 text-gold" /><h2 className="mt-4 font-display text-xl">Henüz sertifika kaydınız yok</h2><p className="mx-auto mt-2 max-w-lg text-sm text-muted-foreground">Sahip olduğunuz bir parsel için sertifikanızı kendiniz oluşturabilirsiniz.</p><Link to="/sertifika-talep" className="btn-gold mt-5 inline-flex rounded-md px-5 py-2.5 text-xs">SERTİFİKA OLUŞTUR</Link></div>}
  ...
  </div></main>{selected && <CertificateModal certificate={selected} onClose={() => setSelected(null)} />}<TrustBar items={SECURITY_TRUST} /><SiteFooter /></div>;
}
