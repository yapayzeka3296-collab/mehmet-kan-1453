...
function Harita() {
  ...
  const availableLayers = useMemo(() => {
    return Array.from(new Set(parcels.map((parcel) => parcel.layer_number).filter((value): value is number => typeof value === "number"))).sort((a, b) => a - b);
  }, [parcels]);
  ...
  const selectCity = (code: CityImageCode) => {
      ...
      <main className="mx-auto max-w-[1800px] px-3 py-7 sm:px-5 lg:px-8 lg:py-10">
        <div className="mb-6 text-center sm:mb-8">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-sm text-muted-foreground sm:text-base">
            Şehrin üzerindeki dijital parsel kubbesini keşfet. Haritayı sürükle, yakınlaştır ve gerçek bir parsele dokun.
          </p>
        </div>
        ...
        <section className="relative overflow-hidden rounded-3xl border border-sky-200/15 bg-slate-950/50 shadow-2xl shadow-black/30">
          <img
            ...
            alt={`${selectedCityMeta.name} şehir manzarası`}
            loading="eager"
            width={1536}
            height={864}
            onError={(event) => {
          ...
          <div className="relative z-10 grid min-h-[760px] lg:min-h-[820px] lg:grid-cols-[280px_1fr]">
            <aside className="order-2 border-t border-white/10 bg-slate-950/72 p-4 backdrop-blur-md lg:order-1 lg:border-r lg:border-t-0 lg:p-5">
              <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-slate-950/55 px-3">
              ...
              <div className="mt-5">
                <div className="mb-2 flex items-center justify-between">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-sky-100/55">Pilot şehirler</p>
                  <MapPin className="h-4 w-4 text-sky-200/55" />
                </div>
                <div className="grid gap-1.5">
                  {filteredCities.map((city) => (
                    <button key={city.code} type="button" onClick={() => selectCity(city.code)} className={`flex items-center gap-2 rounded-xl border px-3 py-2.5 text-left text-sm transition ${selectedCity === city.code ? "border-sky-200/40 bg-sky-200/10 text-white shadow-inner shadow-sky-200/10" : "border-transparent text-white/65 hover:border-white/10 hover:bg-white/5 hover:text-white"}`}>
              ...
              <div className="mt-6 border-t border-white/10 pt-5">
                <div className="mb-3 flex items-center gap-2">
                  <Layers3 className="h-4 w-4 text-sky-200/75" />
                  <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-sky-100/60">Katman ve sektör</p>
                </div>
                <label className="block text-xs text-white/45" htmlFor="map-layer">Katman</label>
                <select id="map-layer" value={layerFilter ?? ""} onChange={(event) => handleLayerChange(event.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-slate-950/75 px-3 py-2.5 text-sm text-white outline-none focus:border-sky-200/35">
                ...
                <select id="map-sector" value={sectorFilter ?? ""} onChange={(event) => setSectorFilter(event.target.value ? Number(event.target.value) : null)} className="mt-2 w-full rounded-xl border border-white/10 bg-slate-950/75 px-3 py-2.5 text-sm text-white outline-none focus:border-sky-200/35">
              ...
              <div className="mt-6 rounded-2xl border border-white/10 bg-slate-950/45 p-4">
                ...
                <div className="mt-3 space-y-2 text-xs">
                  <div className="flex items-center justify-between text-white/70"><span className="flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-sky-300 shadow-[0_0_8px_rgba(92,205,255,.9)]" />Dijital</span><span>199 TL</span></div>
                  <div className="flex items-center justify-between text-white/70"><span className="flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-violet-300 shadow-[0_0_8px_rgba(194,125,255,.9)]" />Elit</span><span>499 TL</span></div>
                  <div className="flex items-center justify-between text-white/70"><span className="flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-amber-300 shadow-[0_0_8px_rgba(255,210,91,.9)]" />Premium</span><span>999 TL</span></div>
                </div>
              </div>

              <div className="mt-4 flex items-start gap-2 rounded-2xl border border-white/10 bg-slate-950/35 p-3 text-[11px] leading-5 text-white/45">
                <Rotate3D className="mt-0.5 h-4 w-4 shrink-0 text-sky-200/60" />
                <span>Haritayı parmağınla sürükle. Mouse tekerleğiyle veya dokunmatik yakınlaştırmayla kubbeye yaklaş.</span>
              </div>
            </aside>

            <div className="order-1 relative min-w-0 lg:order-2">
              <div className="absolute left-4 top-4 z-20 rounded-full border border-white/15 bg-slate-950/55 px-3 py-1.5 text-xs font-medium text-white/80 backdrop-blur-md sm:left-6 sm:top-6">
                {selectedCityMeta.name} · {parcels.length.toLocaleString("tr-TR")} gerçek parsel
              </div>
              ...
              <div className="pointer-events-none absolute inset-x-0 bottom-4 z-20 flex justify-center px-4">
                <div className="rounded-full border border-white/10 bg-slate-950/55 px-4 py-2 text-center text-[10px] text-white/55 shadow-lg backdrop-blur-md">
                  {layerFilter !== null ? `Katman ${layerFilter}` : "Tüm katmanlar"} · {sectorFilter !== null ? `Sektör ${String(sectorFilter).padStart(2, "0")}` : "Tüm sektörler"}
                </div>
              </div>
