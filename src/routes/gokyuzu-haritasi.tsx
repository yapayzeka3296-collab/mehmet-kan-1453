...
import { Layers3, MapPin, Search, ListChecks, X, ShoppingCart } from "lucide-react";
import { z } from "zod";
...
export const Route = createFileRoute("/gokyuzu-haritasi")({
  ...
  head: () => ({ head: undefined, meta: [{ title: "Gökyüzü Haritası — MySkyParcel" }, { name: "description", content: "MySkyParcel dijital parsellerini Google Maps üzerinde keşfet, seç ve detaylarını incele." }, { property: "og:title", content: "Gökyüzü Haritası — MySkyParcel" }, { property: "og:description", content: "MySkyParcel parsellerini gerçek harita üzerinde keşfet." }] }),
  component: Harita,
});
...
function Harita() {
  ...
  const loadViewportParcels = useCallback(async (bounds: ViewportBounds) => {
    ...
    try {
      ...
      const normalized = ((data ?? []) as PublicParcelRow[]).map((parcel) => {
        ...
        return { ...parcel, owner_id: null, tier, tier_price: parcel.tier_price ?? TIER_PRICE[tier], city_name: parcel.city_name ?? selectedCityMeta.name, city_code: parcel.city_code ?? selectedCityMeta.code } as MapParcel;
      });
  ...
  const handleLayerChange = (value: string) => {
    setLayerFilter(value ? Number(value) : null);
    setSectorFilter(null);
  };

  const handleReserved = (reservedParcel: Parcel) => {
    setParcels((current) => current.map((parcel) => parcel.id === reservedParcel.id ? { ...parcel, ...reservedParcel, status: "reserved" } : parcel));
  };
  ...
  const toggleSelectedParcel = (id: string) => {
  ...
  const removeSelected = (id: string) => setSelectedIds((current) => {
    const next = new Set(current);
    next.delete(id);
    return next;
  });
  ...
  const goToPurchase = () => {
    ...
    void navigate({ to: "/parsel-satin-al", search: { parcels: selectedParcelQuery } as never });
  };
    ...
    <main className="mx-auto max-w-[1800px] px-3 py-7 sm:px-5 lg:px-8 lg:py-10">
      ...
      <section className="grid overflow-hidden rounded-3xl border border-sky-200/15 bg-slate-900/70 shadow-2xl shadow-black/30 lg:grid-cols-[280px_1fr]">
        <aside className="order-2 border-t border-white/10 bg-slate-950/90 p-4 backdrop-blur-md lg:order-1 lg:border-r lg:border-t-0 lg:p-5">
          ...
          <div className="mt-5 rounded-2xl border border-white/10 bg-slate-900/65 p-4 text-xs text-white/55"><p className="font-semibold text-white/75">Harita kullanımı</p><p className="mt-2 leading-5">Haritayı parmağınla sürükle veya yakınlaştır. Renkli alanlar MySkyParcel parsellerini gösterir.</p></div>
        </aside>
        <div className="order-1 min-w-0 p-2 sm:p-3 lg:order-2 lg:p-4">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2 px-1 sm:px-2"><div className="rounded-full border border-white/10 bg-slate-950/70 px-3 py-1.5 text-xs text-white/75">{selectedCityMeta.name} · {filteredParcels.length.toLocaleString("tr-TR")} parsel</div><div className="flex items-center gap-2"><button type="button" onClick={toggleMultiSelect} className={`inline-flex items-center gap-2 rounded-full border px-3 py-2 text-xs font-semibold transition ${multiSelect ? "border-cyan-300/60 bg-cyan-300/15 text-cyan-100 shadow-lg shadow-cyan-500/10" : "border-white/15 bg-slate-950/80 text-white/80 hover:border-cyan-300/40 hover:text-white"}`}><ListChecks className="h-4 w-4" />{multiSelect ? "Çoklu Seçim Açık" : "Çoklu Seçim"}</button>{loading && <span className="text-xs text-white/45">Görünen alan yükleniyor...</span>}{error && <span className="text-xs text-red-200">{error}</span>}</div></div>
          <div className="relative"><GoogleParcelMap parcels={filteredParcels} selectedId={selectedId} selectedIds={selectedIds} multiSelect={multiSelect} onSelect={setSelectedId} onToggleSelect={toggleSelectedParcel} onViewportChange={loadViewportParcels} center={selectedCityMeta.center} />
            {multiSelect && selectedParcels.length > 0 && <div className="absolute bottom-4 left-4 right-4 z-10 rounded-2xl border border-cyan-200/20 bg-slate-950/95 p-4 shadow-2xl backdrop-blur-md"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold text-white">{selectedParcels.length} parsel seçildi</p><p className="mt-1 text-xs text-white/50">Seçtiğin parsellerin statü ışığı haritada yanık kalır.</p></div><div className="text-right"><p className="text-xs text-white/50">Toplam</p><p className="text-lg font-bold text-cyan-100">{selectedTotal.toLocaleString("tr-TR")} TL</p></div></div><div className="mt-3 max-h-40 space-y-2 overflow-auto pr-1">{selectedParcels.map((parcel) => <div key={parcel.id} className="flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2"><div className="min-w-0"><p className="truncate text-sm font-medium text-white">{parcel.parcel_number}</p><p className="text-xs text-white/45">{parcel.city_name} · {parcel.tier === "premium" ? "Premium" : parcel.tier === "elite" ? "Elit" : "Dijital"}</p></div><button type="button" onClick={() => removeSelected(parcel.id)} className="rounded-lg p-1.5 text-white/50 hover:bg-white/10 hover:text-white" aria-label={`${parcel.parcel_number} seçimini kaldır`}><X className="h-4 w-4" /></button></div>)}</div><button type="button" onClick={goToPurchase} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-300 px-4 py-3 text-sm font-bold text-slate-950 transition hover:bg-cyan-200 disabled:cursor-not-allowed disabled:opacity-50" disabled={!selectedParcelQuery}><ShoppingCart className="h-4 w-4" />SEÇİLEN PARSELLERİ SATIN AL — {selectedTotal.toLocaleString("tr-TR")} TL</button></div>}
          </div>
        </div>
      </section>
      {selectedParcel && !multiSelect && <ParcelDetailPanel parcel={selectedParcel} onClose={() => setSelectedId(null)} onReserved={handleReserved} />}
    </main>
    <TrustBar /><SiteFooter />
  </div>;
}
