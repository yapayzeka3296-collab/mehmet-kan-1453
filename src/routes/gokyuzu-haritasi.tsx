...
function Harita() {
  ...
  const queryFocusTarget = useMemo<FocusTarget | null>(() => {
    if (!focusParcelId || !Number.isFinite(Number(focusLat)) || !Number.isFinite(Number(focusLng))) return null;
    return { city: selectedCityMeta.center, parcel: { lat: Number(focusLat), lng: Number(focusLng) }, token: `${focusParcelId}:${focusLat}:${focusLng}` };
  }, [focusParcelId, focusLat, focusLng, selectedCityMeta.center]);
  ...
  }, [citySlug, focusParcelQuery, focusLat, focusLng]);
