...
type SkyParcel = {
  id: string; parcel_number: string; status: "available" | "reserved" | "sold" | string;
  price: number | string | null; tier: string; tier_price: number | string | null;
  city_name: string; city_slug: string; latitude: number; longitude: number;
};
...
function SkyScanPage() {
  ...
  useEffect(() => {
    ...
    const loadNearbyParcels = async () => {
      ...
      if (error) { setParcelError(error.message || "Yakındaki parseller alınamadı."); setNearbyParcels([]); }
        ...
        setNearbyParcels(rows.map((parcel) => ({ ...parcel, distance: distanceMeters(location, parcel), bearing: bearingDegrees(location, parcel) })).sort((a, b) => a.distance - b.distance));
      }
  ...
  const startScan = async () => {
    ...
    try {
      if (!window.isSecureContext) throw new Error("Kamera yalnızca HTTPS bağlantısında çalışır. Vercel Preview HTTPS kullanır.");
      ...
      if (!video) { stream.getTracks().forEach((track) => track.stop()); throw new Error("Kamera görüntü alanı hazırlanamadı."); }
      ...
      const message = name === "NotAllowedError" ? "Kamera izni verilmedi. Tarayıcı ayarlarından kamera iznini açın ve tekrar deneyin." :
        name === "NotFoundError" ? "Arka kamera bulunamadı." :
        name === "NotReadableError" ? "Kamera başka bir uygulama tarafından kullanılıyor. Kamerayı kapatıp tekrar deneyin." :
  ...
  const visibleParcels = heading === null ? [] : nearbyParcels
    ...
    .sort((a, b) => a.parcel.distance - b.parcel.distance).slice(0, 12);
        ...
        <div className="relative mt-6 min-h-[72vh] flex-1 overflow-hidden rounded-3xl border border-cyan-300/15 bg-slate-950 shadow-2xl">
          {cameraStarted ? (
            <div className="absolute inset-0 overflow-hidden bg-black">
              <video ref={videoRef} className="absolute inset-0 z-0 h-full w-full bg-black object-cover" playsInline muted autoPlay />
              ...
              <div className="pointer-events-none absolute inset-x-0 top-0 z-20 flex justify-center pt-4"><div className="rounded-full border border-white/20 bg-black/55 px-4 py-2 text-xs font-semibold backdrop-blur-md">🧭 {heading === null ? "Yön algılanıyor…" : `${heading.toFixed(0)}°`} · {visibleParcels.length} parsel</div></div>
              <div className="pointer-events-none absolute inset-0 z-10">
                {visibleParcels.map(({ parcel, angle }) => {
                  const leftPercent = 50 + (angle / CAMERA_FOV_DEGREES) * 100;
                  return <div key={parcel.id} className="absolute top-[38%] -translate-x-1/2 transition-all duration-150" style={{ left: `${leftPercent}%` }}><div className="flex flex-col items-center"><div className="relative flex h-16 w-16 items-center justify-center rounded-2xl border-2 border-cyan-200/80 bg-cyan-400/20 shadow-[0_0_30px_rgba(34,211,238,0.65)] backdrop-blur-sm"><div className="absolute inset-2 rotate-45 rounded-lg border border-white/70" /><span className="relative text-[9px] font-black tracking-wider">SKY</span></div><div className="mt-2 whitespace-nowrap rounded-xl border border-white/20 bg-black/65 px-3 py-2 text-center shadow-xl backdrop-blur-md"><p className="text-xs font-bold text-cyan-100">✦ {parcel.parcel_number}</p><p className="mt-0.5 text-[10px] text-white/75">{formatDistance(parcel.distance)} · {parcel.bearing.toFixed(0)}°</p></div></div></div>;
                })}
              </div>
