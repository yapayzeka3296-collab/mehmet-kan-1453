...
function SkyScanPage() {
  ...
  const startSensors = async () => {
    ...
    try {
      ...
      // iOS requires this call to happen directly from the button gesture.
      // Android Chrome normally has no requestPermission() method and starts after activation.
  ...
  useEffect(() => {
    const read = (event: DeviceOrientationWithCompass, source: "absolute" | "relative") => {
      ...
      const compass = Number.isFinite(rawCompass) ? ((rawCompass! % 360) + 360) % 360 : undefined;
      if (source === "relative" && orientationRef.current?.source === "absolute" && compass == null) return;
      const sample: OrientationSample = {
        alpha: event.alpha,
        beta: event.beta,
        gamma: event.gamma,
        absolute: source === "absolute" || event.absolute,
        compass,
        source: compass != null ? "compass" : source,
      };
      orientationRef.current = sample;
      if (compass != null) setHeadingText(`Pusula: ${Math.round(compass)}°`);
      else if (sample.absolute) setHeadingText(`Pusula: ${Math.round(((sample.alpha % 360) + 360) % 360)}°`);
    };
  ...
  useEffect(() => {
    ...
    const createParcel = (parcel: SkyParcel) => {
      ...
      // World coordinates are ENU: +X east, +Y up, +Z north.
      // Three.js camera looks down -Z, so north is rendered at -world.z.
      ...
      const beacon = new THREE.Mesh(new THREE.CylinderGeometry(2, 4, 45, 10), new THREE.MeshStandardMaterial({ color: 0xf4c65d, emissive: 0x9b6718, emissiveIntensity: 0.9 }));
      beacon.position.y = 25;
      group.add(beacon);
      const ring = new THREE.Mesh(new THREE.TorusGeometry(size * 0.68, 1.1, 8, 32), new THREE.MeshBasicMaterial({ color: 0xffd76a }));
    ...
    const animate = () => {
      ...
      if (sensorEnabledRef.current && sample) {
        // Use the physical compass heading for yaw whenever available.
        // This prevents parcels from behaving like camera-fixed overlays.
        const heading = sample.compass ?? (sample.absolute ? sample.alpha : null);
        const yaw = heading == null ? null : THREE.MathUtils.degToRad(-heading);
        if (yaw != null) {
          const pitch = THREE.MathUtils.degToRad(-(sample.beta - 90));
          const roll = THREE.MathUtils.degToRad(-sample.gamma);
          const screenAngle = THREE.MathUtils.degToRad(screen.orientation?.angle ?? 0);
          const worldQuaternion = new THREE.Quaternion().setFromEuler(new THREE.Euler(pitch, yaw, roll, "YXZ"));
          if (screenAngle) worldQuaternion.multiply(new THREE.Quaternion(0, 0, Math.sin(-screenAngle / 2), Math.cos(-screenAngle / 2)));
          camera.quaternion.slerp(worldQuaternion, 0.18);
        }
      }
