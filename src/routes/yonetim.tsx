...
function Admin() {
  ...
  async function refresh() { setRefreshKey((current) => current + 1); await load(); }
...
function Dashboard({ stats, refreshKey }: { stats: Stats | null; refreshKey: number }) {
  ...
  useEffect(() => { void (async () => { const { data } = await supabaseBrowser.from("admin_notifications").select("id,title,message,is_read,created_at").order("created_at", { ascending: false }).limit(20); setNotifications((data ?? []) as Row[]); })(); }, [refreshKey]);
  useEffect(() => { void (async () => { setSiteStatsError(""); const { data, error: rpcError } = await supabaseBrowser.rpc("admin_site_statistics"); if (rpcError) { setSiteStatsError(rpcError.message); return; } setSiteStats(data as Stats); })(); }, [refreshKey]);
