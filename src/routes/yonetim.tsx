      ...
      <aside className="border-r border-border bg-navy-deep p-4">
        <div className="flex items-center gap-2 px-2 py-3">
          ...
          <div>
            ...
            <div className="text-[8px] tracking-[0.25em] text-muted-foreground">
              YÖNETİM PANELİ
            </div>
          </div>
        </div>
      ...
      <main className="min-w-0">
        ...
        <div className="space-y-6 p-4 lg:p-6">
          ...
          {module === "dashboard" ? (
            ...
            <ModuleView module={module} rows={rows} />
          )}
        </div>
      </main>
    </div>
...
function ModuleView({
  ...
  if (module === "users") {
    return <DataTable rows={rows} columns={["id", "email", "full_name", "role", "created_at"]} />;
  }

  if (module === "parcels") {
    return <DataTable rows={rows} columns={["id", "parcel_number", "status", "tier", "price", "owner_id"]} />;
  }

  if (module === "orders") {
    return <DataTable rows={rows} columns={["id", "user_id", "parcel_id", "amount", "currency", "status", "created_at"]} />;
  }
...
function DataTable({
  ...
  return (
    <div className="panel overflow-auto p-5">
        ...
        <tbody>
          {rows.map((row, index) => (
            <tr key={row.id ?? index} className="border-b border-border/50">
              {columns.map((column) => (
                <td key={column} className="max-w-[260px] truncate p-2">
                  {column.endsWith("_at") && row[column]
                    ? new Date(row[column]).toLocaleString("tr-TR")
                    : String(row[column] ?? "—")}
                </td>
              ))}
            </tr>
      ...
      {rows.length === 0 && (
        <p className="py-4 text-sm text-muted-foreground">Kayıt bulunmuyor.</p>
      )}
    </div>
  );
}
