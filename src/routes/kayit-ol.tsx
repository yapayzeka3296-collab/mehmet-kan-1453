...
export const Route = createFileRoute("/kayit-ol")({
  head: () => ({ meta: [{ title: "Kayıt Ol — MySkyParcel" }, { name: "description", content: "Ücretsiz hesap oluştur, gökyüzündeki parselini seç ve sertifikanı al." }, { property: "og:title", content: "Kayıt Ol — MySkyParcel" }, { property: "og:description", content: "MySkyParcel'e ücretsiz üye ol." }] }),
  component: KayitOl,
});
...
function KayitOl() {
  ...
  return (
    <div className="starfield min-h-screen"><SiteHeader /><main className="relative overflow-hidden">
      ...
      <div className="relative mx-auto grid max-w-[1600px] gap-10 px-4 py-14 lg:grid-cols-2 lg:px-8">
        ...
        <div className="panel min-w-0 p-6 sm:p-10"><h2 className="text-center font-display text-3xl">KAYIT OL</h2>
          <form className="mt-8 space-y-5" onSubmit={onSubmit}>
            <div><label className="text-xs text-muted-foreground" htmlFor="ad">Ad Soyad</label><div className="mt-2 flex items-center gap-3 rounded-md border border-input bg-background/50 px-3 focus-within:border-gold"><User className="h-4 w-4 shrink-0 text-muted-foreground" /><input id="ad" name="name" type="text" placeholder="Ahmet Yılmaz" autoComplete="name" value={name} onChange={(e) => setName(e.target.value)} className="min-w-0 flex-1 bg-transparent py-3 text-sm outline-none" /></div></div>
      ...
      </div><TrustBar items={SECURITY_TRUST} />
    </main><SiteFooter /></div>
  );
}
