...
export const Route = createFileRoute("/hakkimizda")({
  head: () => ({ meta: [{ title: "Hakkımızda — MySkyParcel" }, { name: "description", content: "MySkyParcel'in amacı, çalışma biçimi, sınırları ve sık sorulan sorular." }, { property: "og:title", content: "Hakkımızda — MySkyParcel" }, { property: "og:description", content: "MySkyParcel'in amacı, çalışma biçimi ve sık sorulan sorular." }] }),
...
const FAQS = [
  ...
  { q: "81 milyon parselin her biri gerçekten benzersiz mi?", a: "Evet. MySkyParcel'in parsel modeli toplam 81 milyon benzersiz parsel üzerine kuruludur. Her parsel kendine özel bir parsel koduyla tanımlanır. Bir parsel yalnızca bir kişi tarafından satın alınabilir; satın alma tamamlandığında ilgili parsel sahibinin hesabına kaydedilir ve aynı parsel kodu başka bir parsel için yeniden oluşturulmaz." },
...
function Hakkimizda() {
  return (
    <div className="starfield min-h-screen bg-cover bg-center bg-fixed" style={{ backgroundImage: "url('/hero-background.jpg')" }}>
      <SiteHeader />
      <main>
        <section className="relative overflow-hidden">
          <div className="relative mx-auto max-w-3xl px-4 py-20 text-center lg:px-8">
            ...
            <p className="mt-5 text-sm leading-7 text-muted-foreground">MySkyParcel, gerçek dünyadaki taşınmazlarla karıştırılmaması gereken, dijital ve sembolik bir gökyüzü parsel deneyimi sunan bir platformdur. Kullanıcılar etkileşimli harita üzerinden sembolik parselleri keşfedebilir, uygun bir parsel seçebilir ve satın alınan hizmet kapsamında dijital koleksiyonlarını oluşturabilir.</p>
          </div>
        </section>
        <section className="mx-auto grid max-w-[1600px] gap-6 px-4 py-14 lg:grid-cols-3 lg:px-8">{[{ icon: Star, t: "MİSYONUMUZ", d: "Dijital dünyada kişisel anlam taşıyan, sade ve güvenli bir parsel seçme ve koleksiyon deneyimi sunmak." }, { icon: Globe, t: "VİZYONUMUZ", d: "Türkiye'nin şehirlerini dijital bir gökyüzü haritası deneyiminde bir araya getirerek geniş bir dijital koleksiyon alanı oluşturmak." }, { icon: Heart, t: "DEĞERLERİMİZ", d: "Şeffaf iletişim, kullanıcı güvenliği, açık ürün kapsamı ve dijital deneyimin sınırlarını doğru anlatmak." }].map((c) => <article key={c.t} className="panel p-7"><c.icon className="h-8 w-8 text-gold" /><h2 className="mt-4 font-display text-lg">{c.t}</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">{c.d}</p></article>)}</section>
        <section className="mx-auto max-w-[1200px] px-4 pb-14 lg:px-8"><div className="panel border-gold/20 p-6 sm:p-8"><h2 className="font-display text-xl text-gold">PLATFORMUN HUKUKİ KAPSAMI</h2><p className="mt-4 text-sm leading-7 text-muted-foreground">MySkyParcel'de kullanılan "parsel", "sahiplik", "koleksiyon" ve "sertifika" ifadeleri platformun dijital hizmeti içindeki anlamlarıyla kullanılır. Bu ifadeler gerçek bir taşınmazın mülkiyetini, tapu hakkını, arsa tahsisini veya başka bir ayni hakkı temsil etmez. MySkyParcel bir tapu sicili veya resmi taşınmaz kayıt sistemi değildir. Kullanıcı, işlem yapmadan önce satın aldığı ürünün ve hizmetin kapsamını, fiyatını ve yürürlükteki sözleşme koşullarını incelemelidir.</p><p className="mt-4 text-xs leading-6 text-muted-foreground/80">Bu açıklama kullanıcı bilgilendirmesi amacıyla hazırlanmıştır ve somut bir hukuki görüş veya hukuki danışmanlık yerine geçmez. Uygulanabilir mevzuat ve sözleşme hükümleri her zaman önceliklidir.</p></div></section>
        <section id="sss" className="mx-auto max-w-[1200px] scroll-mt-24 px-4 pb-16 lg:px-8"><div className="text-center"><h2 className="font-display text-3xl font-bold sm:text-4xl">SIK SORULAN SORULAR</h2><p className="mx-auto mt-3 max-w-2xl text-sm text-muted-foreground">MySkyParcel'in çalışma biçimi, parseller, sertifikalar ve platformun kapsamı hakkında en sık sorulan sorular.</p></div><div className="mt-8 grid gap-3">{FAQS.map((item) => <details key={item.q} className="group rounded-xl border border-border bg-background/40 p-5"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-medium text-slate-100 [&::-webkit-details-marker]:hidden"><span>{item.q}</span><ChevronDown className="h-5 w-5 shrink-0 text-gold transition-transform group-open:rotate-180" /></summary><p className="mt-4 max-w-4xl text-sm leading-7 text-muted-foreground">{item.a}</p></details>)}</div></section>
      </main>
      <TrustBar /><SiteFooter />
    </div>
  );
