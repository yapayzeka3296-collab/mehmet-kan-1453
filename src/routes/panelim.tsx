...
function Panelim() {
  ...
  useEffect(() => {
    ...
    const loadDashboard = async () => {
      ...
      if (orderResult.error) { console.error("Siparişler yüklenemedi", orderResult.error); errors.push("Siparişlerim"); }
  ...
  const stats = { parcels: parcels.length, certificates: certificates.length, orders: orders.length, favorites: "—" };
      ...
      <main className="mx-auto grid max-w-[1600px] gap-6 px-4 py-8 lg:grid-cols-[260px_1fr] lg:px-8">
        ...
        <section className="min-w-0" aria-label="Kullanıcı paneli">
          ...
          <div className="mt-6 grid gap-6 xl:grid-cols-2">
            <section className="panel p-6"><div className="flex items-center justify-between gap-4"><h2 className="font-display text-base tracking-[0.06em]">SON PARSELLERİM</h2><span className="text-xs text-muted-foreground">{dataLoading ? "…" : parcels.length}</span></div><div className="mt-6 space-y-3">
            ...
            <section className="panel p-6"><div className="flex items-center justify-between gap-4"><h2 className="font-display text-base tracking-[0.06em]">SON SİPARİŞLERİM</h2><span className="text-xs text-muted-foreground">{dataLoading ? "…" : orders.length}</span></div><div className="mt-6 space-y-3">
