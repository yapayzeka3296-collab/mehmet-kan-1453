        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[minmax(0,1fr)_440px]">
          <section className="panel grid gap-5 p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="block sm:col-span-2"><span className="text-xs text-muted-foreground">Sertifikada Görünecek Ad Soyad</span><input id="certificate-name" value={certificateName} onChange={(e) => setCertificateName(e.target.value)} placeholder="Ahmet Yılmaz" autoComplete="name" className="mt-1.5 w-full rounded-md border border-input bg-background/50 px-3 py-2.5 text-sm outline-none focus:border-gold" /></label>
              <label className="block sm:col-span-2"><span className="text-xs text-muted-foreground">Mesajınız (opsiyonel)</span><textarea id="certificate-message" value={message} onChange={(e) => setMessage(e.target.value.slice(0, MAX_MESSAGE_LENGTH))} rows={4} maxLength={MAX_MESSAGE_LENGTH} placeholder="Sevdiklerinize özel notunuzu yazın..." className="mt-1.5 w-full resize-none rounded-md border border-input bg-background/50 px-3 py-2.5 text-sm outline-none focus:border-gold" /><span className="mt-1 block text-right text-[10px] text-muted-foreground">{message.length}/{MAX_MESSAGE_LENGTH}</span></label>
            </div>
          ...
          <aside className="panel h-fit p-4 sm:p-6 lg:sticky lg:top-6">
            <div className="flex items-center justify-between"><h2 className="font-display text-base">SERTİFİKA ÖNİZLEME</h2><span className="text-[10px] text-muted-foreground">PREMİUM</span></div>
            <div className="relative mt-4 overflow-hidden rounded-lg border border-gold/30 bg-white shadow-lg">
              <img src={certificateImage} alt="Premium sertifika şablonu" width={1600} height={1067} className="block aspect-[1600/1067] w-full object-cover" />
              <div className="pointer-events-none absolute inset-x-[12%] bottom-[7%] text-center">
                <p className="mx-auto max-w-[85%] truncate text-[clamp(8px,1.4vw,15px)] font-semibold text-black/80">{certificateCreated ? certificateName : "Ad Soyad"}</p>
                <p className="mx-auto mt-1 max-w-[78%] break-words text-[clamp(6px,0.9vw,10px)] leading-tight text-black/70">{certificateCreated && message ? message : "Özel mesajınız QR kodunun altında burada görünecek."}</p>
              </div>
            </div>
            <p className="mt-3 text-center text-[11px] leading-5 text-muted-foreground">Mesajınız sertifika şablonundaki QR kodunun altında, okunabilir ancak tasarımı kapatmayacak küçük boyutta yerleştirilecektir.</p>
