...
import { ArrowRight, Check, FileBadge2, Loader2, Sparkles } from "lucide-react";
...
export const Route = createFileRoute("/parsel-satin-al")({
  ...
  head: () => ({ meta: [{ title: "Parsel Satın Al — MySkyParcel" }, { name: "description", content: "İstediğiniz sayıda sembolik parsel seçin ve yalnızca bir parsel için sertifika talep edin." }] }),
  component: SatinAl,
});
...
function SatinAl() {
  ...
  useEffect(() => {
    ...
    async function validateParcels() {
      ...
      const { data, error } = await supabaseBrowser.from("parcels").select("id,tier,tier_price,status").in("id", ids);
      if (!active) return;
      if (error) { setValidationError("Parseller doğrulanamadı. Lütfen tekrar deneyin."); setLoading(false); return; }
      ...
      if (missing.length || unavailable.length || rows.some((row) => !row.tier || !["digital", "elite", "premium"].includes(row.tier))) {
        setValidationError(unavailable.length ? `Şu parseller artık satışa uygun değil: ${unavailable.map((row) => row.id).join(", ")}` : "Seçilen parsellerin satış bilgileri doğrulanamadı.");
        setItems([]); setLoading(false); return;
      }
      const nextItems = ids.map((id) => { const row = byId.get(id)!; const tier = row.tier!; return { id, tier, price: Number(row.tier_price ?? PRICES[tier]) }; });
      setItems(nextItems);
  ...
  useEffect(() => {
    if (items.length && certificateParcel && typeof window !== "undefined") window.localStorage.setItem("myskyparcel_purchase_plan", JSON.stringify({ items, certificateParcel }));
  }, [items, certificateParcel]);

  function handleContinue() {
    if (!items.length || !certificateParcel || !accepted || !physicalReady) return;
    if (typeof window !== "undefined") {
      window.localStorage.setItem("myskyparcel_purchase_plan", JSON.stringify({ items, certificateParcel }));
      if (PHYSICAL[certificateTier]) window.localStorage.setItem("myskyparcel_delivery", JSON.stringify({ name: name.trim(), phone: phone.trim(), address: address.trim(), city: city.trim(), district: district.trim() }));
    }
    void navigate({ to: "/odeme", search: { parcels: items.map((item) => item.id).join(","), certificateParcel } });
  }
