        ...
        <ol className="mx-auto mt-8 flex max-w-3xl flex-wrap justify-center gap-4">
          {STEPS.map((s, i) => (
            <li key={s} className="flex items-center gap-2 text-xs">
              <span className={`grid h-7 w-7 place-items-center rounded-full border ${i === 0 ? "border-gold text-gold" : "border-border text-muted-foreground"}`}>{i + 1}</span>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_360px]">
          <section className="panel grid gap-5 p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="block sm:col-span-2"><span className="text-xs text-muted-foreground">Sertifikada Görünecek Ad Soyad</span><input placeholder="Ahmet Yılmaz" className="mt-1.5 w-full rounded-md border border-input bg-background/50 px-3 py-2.5 text-sm outline-none focus:border-gold" /></label>
              <label className="block sm:col-span-2"><span className="text-xs text-muted-foreground">Mesajınız (opsiyonel)</span><textarea rows={4} placeholder="Sevdiklerinize özel notunuzu yazın..." className="mt-1.5 w-full rounded-md border border-input bg-background/50 px-3 py-2.5 text-sm outline-none focus:border-gold" /></label>
            </div>
            <Link to="/paketler" className="btn-gold inline-flex w-fit items-center gap-2 rounded-md px-6 py-3 text-[11px]">DEVAM ET <ArrowRight className="h-4 w-4" /></Link>
          </section>

          <aside className="panel h-fit p-6">
            <h2 className="font-display text-base">SİPARİŞ ÖZETİ</h2>
            <img src={certDigital} alt="Sertifika önizleme" loading="lazy" width={800} height={1000} className="mt-4 h-52 w-full rounded-lg object-contain" />
