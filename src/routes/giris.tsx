...
const FEATURES = [
  { icon: Globe, big: "10.000+", label: "Parsel" },
  { icon: Layers, big: "10 Katman", label: "100 Sektör" },
  { icon: ShieldCheck, big: "Güvenli Altyapı", label: "256 Bit SSL" },
  { icon: Lock, big: "%100 Mülkiyet", label: "Sertifikalı" },
];
        ...
        <div className="relative mx-auto grid max-w-[1600px] gap-10 px-4 py-14 lg:grid-cols-2 lg:px-8">
          <div className="min-w-0">
            ...
            <p className="mt-5 max-w-md text-sm text-muted-foreground">
              Kendinize veya sevdiklerinize unutulmaz bir hediye verin. Gökyüzündeki yerinizi seçin,
              sertifikanızı alın ve bu eşsiz deneyimin bir parçası olun.
            </p>
            <ul className="mt-10 flex flex-wrap gap-8">
              {FEATURES.map((f) => (
                <li key={f.big} className="min-w-0">
                  <f.icon className="h-7 w-7 text-gold" />
                  <p className="mt-2 text-sm font-semibold">{f.big}</p>
          ...
          <div className="panel min-w-0 p-6 sm:p-10">
            <div className="text-center">
              ...
              <h2 className="mt-4 font-display text-3xl">GİRİŞ YAP</h2>
              <p className="mt-3 text-sm text-muted-foreground">
                Hesabınıza giriş yaparak parsellerinizi yönetin ve sertifikalarınıza ulaşın.
              </p>
            </div>
            ...
            <form className="mt-8 space-y-5" onSubmit={(e) => e.preventDefault()}>
              ...
              <div>
                <label className="text-xs text-muted-foreground" htmlFor="pass">
                  Şifreniz
                </label>
                <div className="mt-2 flex items-center gap-3 rounded-md border border-input bg-background/50 px-3 focus-within:border-gold">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
                <label className="flex items-center gap-2">
                  <input type="checkbox" defaultChecked className="accent-[oklch(0.78_0.13_82)]" />
                  Beni hatırla
                </label>
                <Link to="/sifremi-unuttum" className="text-gold hover:underline">
                  Şifremi unuttum?
                </Link>
              </div>

              <button className="btn-gold flex w-full items-center justify-center gap-3 rounded-md py-3.5 text-sm">
                GİRİŞ YAP <ArrowRight className="h-4 w-4" />
              </button>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <button
                  type="button"
                  className="rounded-md border border-border py-3 text-sm transition-colors hover:border-gold"
                >
