        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[360px_1fr]">
          <aside className="panel grid content-start gap-5 p-6">
            ...
            {[
              ...
              <div key={c.t} className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0"><p className="text-xs text-muted-foreground">{c.t}</p><p className="break-words text-sm">{c.v}</p></div>
              </div>
            ))}
          </aside>
          ...
          <form className="panel grid gap-5 p-6" onSubmit={handleSubmit}>
            <div className="grid gap-5 sm:grid-cols-2">
              <label className="block"><span className="text-xs text-muted-foreground">Ad Soyad</span><input required name="name" autoComplete="name" className="mt-1.5 w-full rounded-md border border-input bg-background/50 px-3 py-2.5 text-sm outline-none focus:border-gold" /></label>
              <label className="block"><span className="text-xs text-muted-foreground">E-posta</span><input required name="email" type="email" autoComplete="email" className="mt-1.5 w-full rounded-md border border-input bg-background/50 px-3 py-2.5 text-sm outline-none focus:border-gold" /></label>
            </div>
            <label className="block"><span className="text-xs text-muted-foreground">Konu</span><input name="subject" className="mt-1.5 w-full rounded-md border border-input bg-background/50 px-3 py-2.5 text-sm outline-none focus:border-gold" /></label>
            <label className="block"><span className="text-xs text-muted-foreground">Mesajınız</span><textarea required name="message" rows={6} className="mt-1.5 w-full rounded-md border border-input bg-background/50 px-3 py-2.5 text-sm outline-none focus:border-gold" /></label>
            <input name="website" tabIndex={-1} autoComplete="off" aria-hidden="true" className="hidden" />
            <button type="submit" disabled={status === "sending"} className="btn-gold w-fit rounded-md px-8 py-3 text-[11px] disabled:cursor-not-allowed disabled:opacity-60">{status === "sending" ? "GÖNDERİLİYOR..." : "GÖNDER"}</button>
            {statusText && <p role="status" className={`text-sm ${status === "success" ? "text-emerald-400" : "text-red-400"}`}>{statusText}</p>}
          </form>
        </div>
      </main>
      <TrustBar />
