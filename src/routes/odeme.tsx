...
export const Route = createFileRoute("/odeme")({
  validateSearch: (search: Record<string, unknown>) => ({ parcels: typeof search.parcels === "string" ? search.parcels : "", certificateParcel: typeof search.certificateParcel === "string" ? search.certificateParcel : "" }),
...
function Odeme() {
  ...
  const selectedParcels = useMemo(() => Array.from(new Set(parcels.split(",").map((value) => value.trim()).filter(Boolean))), [parcels]);
  ...
  useEffect(() => {
    ...
    async function verify() {
      ...
      const { data, error: queryError } = await supabaseBrowser.from("parcels").select("id,tier,tier_price,status").in("id", selectedParcels);
      if (!active) return;
      if (queryError) { setError("Parseller ödeme öncesinde doğrulanamadı."); setVerified([]); setLoading(false); return; }
      const rows = (data ?? []) as Array<{ id: string; tier: Tier | null; tier_price: number | null; status: string }>;
      const byId = new Map(rows.map((row) => [row.id, row]));
      const missing = selectedParcels.filter((id) => !byId.has(id));
      const unavailable = rows.filter((row) => row.status !== "available");
      if (missing.length || unavailable.length || rows.some((row) => !row.tier || !["digital", "elite", "premium"].includes(row.tier))) {
        setError(unavailable.length ? `Ödeme durduruldu. Satışa uygun olmayan parsel: ${unavailable.map((row) => row.id).join(", ")}` : "Parsel paket bilgileri doğrulanamadı.");
        setVerified([]); setLoading(false); return;
      }
      setVerified(selectedParcels.map((id) => { const row = byId.get(id)!; const tier = row.tier!; return { id, tier, price: Number(row.tier_price ?? PACKAGES[tier].price) }; }));
      setLoading(false);
    }
    void verify();
    return () => { active = false; };
  ...
  const groups = useMemo(() => (["digital", "elite", "premium"] as Tier[]).map((tier): PaymentGroup | null => {
    const tierItems = verified.filter((item) => item.tier === tier);
    if (!tierItems.length) return null;
    return { ...PACKAGES[tier], tier, count: tierItems.length, parcels: tierItems.map((item) => item.id) };
  }).filter((group): group is PaymentGroup => Boolean(group)), [verified]);
  ...
  useEffect(() => {
    ...
    try { window.localStorage.setItem("myskyparcel_checkout_plan", JSON.stringify({ items: verified, certificateParcel, groups, total })); } catch { /* storage unavailable */ }
  }, [verified, certificateParcel, groups, total, certificateIsSelected]);
  ...
  if (!verified.length || !certificateIsSelected) return <div className="starfield min-h-screen"><SiteHeader /><main className="mx-auto max-w-3xl px-4 py-16 lg:px-8"><div className="panel p-8 text-center"><h1 className="font-display text-3xl font-bold">ÖDEME HAZIRLANAMADI</h1><p className="mt-3 text-sm text-muted-foreground">{error ?? (!certificateIsSelected ? "Sertifika için bir parsel seçimi doğrulanamadı." : "Seçilen parseller doğrulanamadı.")}</p><button type="button" onClick={() => void navigate({ to: "/parsel-satin-al", search: { parcels: selectedParcels.join(","), certificateParcel: certificateIsSelected ? certificateParcel : selectedParcels[0] } })} className="mt-6 inline-flex items-center gap-2 rounded-md border border-border px-6 py-3 text-xs"><ArrowLeft className="h-4 w-4" /> BİLGİLERE DÖN</button></div></main><SiteFooter /></div>;

  return <div className="starfield min-h-screen"><SiteHeader /><main className="mx-auto max-w-[1200px] px-4 py-12 lg:px-8"><h1 className="text-center font-display text-4xl font-bold sm:text-5xl">ÖDEME PLANI</h1><p className="mx-auto mt-3 max-w-3xl text-center text-sm text-muted-foreground">Seçtiğiniz {verified.length} parsel sipariş planında korunur. Sertifika yalnızca <strong className="text-foreground">{certificateParcel}</strong> için talep edilir.</p><div className="mt-10 grid gap-6 lg:grid-cols-[1fr_360px]"><section className="panel p-6 sm:p-8"><div className="mb-6 flex items-center gap-3"><Lock className="h-5 w-5 text-gold" /><div><h2 className="font-display text-lg">İYZİCO LİNK İLE ÖDEME</h2><p className="text-xs text-muted-foreground">Kart bilgileri MySkyParcel üzerinde alınmaz; ödeme iyzico'nun güvenli sayfasında yapılır.</p></div></div><div className="rounded-xl border border-gold/20 bg-gold/[0.04] p-5"><p className="text-sm font-semibold text-gold">Doğrulanmış çoklu parsel siparişi</p><p className="mt-2 text-xs leading-5 text-muted-foreground">Paket türleri Supabase'deki gerçek parsel kaydından doğrulandı. Kullanıcının URL veya tarayıcı verisini değiştirerek paket türünü değiştirmesine güvenilmez.</p></div><div className="mt-5 space-y-4">{groups.map((group) => <div key={group.tier} className="rounded-xl border border-border p-5"><div className="flex items-center justify-between gap-4"><div><p className="font-semibold">{group.name}</p><p className="mt-1 text-xs text-muted-foreground">{group.count} parsel × {group.price.toLocaleString("tr-TR")} TL</p></div><p className="font-display text-xl text-gold">{(group.price * group.count).toLocaleString("tr-TR")} TL</p></div><div className="mt-3 text-xs text-muted-foreground">{group.parcels.join(", ")}</div><div className="mt-4 flex items-center justify-between gap-3"><span className="text-[11px] text-muted-foreground">{group.count > 1 ? `${group.count} ayrı başarılı Link ödemesi gerekir.` : "1 başarılı Link ödemesi gerekir."}</span>{group.link ? <a href={group.link} target="_blank" rel="noopener noreferrer" className="btn-gold inline-flex shrink-0 items-center gap-2 rounded-md px-5 py-3 text-[11px]"><Lock className="h-4 w-4" /> ÖDE <ExternalLink className="h-4 w-4" /></a> : <button disabled className="btn-gold rounded-md px-5 py-3 text-[11px] opacity-50">LİNK BEKLENİYOR</button>}</div></div>)}</div><div className="mt-6 rounded-xl border border-border p-5 text-xs text-muted-foreground"><p className="font-semibold text-foreground">Ödeme ve sipariş kaydı</p><p className="mt-2">İyzico Link ödemeleri bu sayfadan otomatik doğrulanmaz. Her Link ödemesinin iyzico panelinde başarılı olduğunun kontrol edilmesi ve ardından sipariş/parsel durumunun kesinleştirilmesi gerekir. Otomatik tek seferlik sepet, ödeme sonucu doğrulama ve atomik parsel satışı için sunucu taraflı ödeme entegrasyonu gerekir.</p></div>{missingLink && <div className="mt-4 rounded-xl border border-destructive/40 bg-destructive/5 p-4 text-xs">Seçilen paketlerden en az birinin canlı iyzico Link'i Vercel ortam değişkenine eklenmemiş.</div>}</section><aside className="panel h-fit p-6 lg:sticky lg:top-6"><h2 className="font-display text-base">SİPARİŞ ÖZETİ</h2><div className="mt-5 space-y-3 text-sm"><div className="flex justify-between"><span className="text-muted-foreground">Parsel sayısı</span><span>{verified.length}</span></div><div className="flex justify-between"><span className="text-muted-foreground">Sertifika</span><span>{certificateParcel}</span></div>{groups.map((group) => <div key={group.tier} className="flex justify-between"><span className="text-muted-foreground">{group.name} × {group.count}</span><span>{(group.price * group.count).toLocaleString("tr-TR")} TL</span></div>)}<div className="border-t border-border pt-4 flex justify-between"><span className="font-semibold">Toplam</span><span className="font-display text-2xl text-gold">{total.toLocaleString("tr-TR")} TL</span></div></div><div className="mt-5 rounded-lg border border-gold/20 bg-gold/[0.04] p-4 text-xs text-muted-foreground"><ShieldCheck className="mb-2 h-4 w-4 text-gold" /><p>Parseller ödeme doğrulanana kadar kesin olarak satılmış kabul edilmez.</p></div><button type="button" onClick={() => void navigate({ to: "/parsel-satin-al", search: { parcels: selectedParcels.join(","), certificateParcel } })} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-md border border-border px-5 py-3 text-xs"><ArrowLeft className="h-4 w-4" /> BİLGİLERE DÖN</button></aside></div></main><SiteFooter /></div>;
}
