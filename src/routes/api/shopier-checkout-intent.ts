...
export async function POST({ request }: { request: Request }) {
  try {
    ...
    const { data: { user }, error: authError } = await supabase.auth.getUser(token); if (authError || !user) return json({ ok: false, reason: 'unauthenticated' }, 401);
    ...
    const { data: orders, error: orderError } = await supabase.rpc('create_shopier_checkout_orders', { p_parcel_ids: parcelIds });
    ...
    const { error: updateError } = await supabase.from('shopier_checkout_intents').update({ status: 'redirected', shopier_product_id: checkout.productId, checkout_url: checkout.paymentUrl }).eq('id', intent.id); if (updateError) return json({ ok: false, reason: 'checkout_mapping_failed' }, 500);
