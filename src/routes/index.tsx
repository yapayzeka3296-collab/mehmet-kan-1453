...
function Index() {
  ...
  return (
    <div className="starfield min-h-screen">
      <SiteHeader />
      <main>
        <section className="hero-sky-parcels relative min-h-[620px] overflow-hidden bg-cover bg-center bg-no-repeat lg:min-h-[700px]" style={{ backgroundImage: "url('/hero-background.jpg')" }}>
        ...
        <section className="mx-auto grid max-w-[1600px] gap-6 px-4 pb-8 lg:px-8 xl:grid-cols-[3fr_1fr]"><div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{CERTIFICATE_PACKAGES.map((p) => <article key={p.id} className="panel flex min-w-0 flex-col p-6"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2"><div className="min-w-0 min-h-[248px]"><h3 className="font-display text-lg">{p.name}</h3><ul className="mt-4 space-y-2 text-xs">{p.features.map((f)=><li key={f} className="flex items-start gap-2 text-muted-foreground"><Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-gold"/><span>{f}</span></li>)}</ul></div><div className="flex h-40 items-center justify-center"><img src={p.img} alt={p.name} loading="lazy" width={1000} height={707} className="h-40 w-full rounded-lg object-contain" onError={(event) => { event.currentTarget.style.display = "none"; }}/></div></div><p className="mt-6 font-display text-3xl text-gold">{p.price.toLocaleString("tr-TR")} <span className="text-base">TL</span></p><Link to="/paketler" className="btn-gold mt-auto pt-2 inline-flex min-h-10 items-center justify-center rounded-md px-5 py-2.5 text-[11px]">HEMEN İNCELE</Link></article>)}</div><div className="grid content-start gap-6"><div className="panel p-6"><h3 className="font-display text-base">SERTİFİKA DOĞRULA</h3><p className="mt-2 text-xs text-muted-foreground">Parsel kodunuzu girerek sertifikanızın geçerliliğini kontrol edebilirsiniz.</p><form onSubmit={handleCertificateSubmit} className="mt-4 flex flex-col gap-2 sm:flex-row"><label htmlFor="homepage-certificate-code" className="sr-only">Sertifika numarası</label><input id="homepage-certificate-code" value={certificateCode} onChange={(event) => setCertificateCode(event.target.value)} aria-label="Sertifika numarası" placeholder="Sertifika numaranızı girin (Örn: SP-GZT-0004207)" autoComplete="off" className="min-w-0 w-full rounded-md border border-input bg-background/60 px-3 py-2 text-[11px] uppercase outline-none focus:border-gold"/><button type="submit" className="btn-gold inline-flex w-full shrink-0 items-center justify-center rounded-md px-4 py-2 text-[11px] sm:w-auto">DOĞRULA</button></form></div><div className="panel p-6"><h3 className="font-display text-base">GÜVENLİ ALIŞVERİŞ</h3><p className="mt-2 text-xs text-muted-foreground">Güvenlik ve ödeme altyapısı yayın öncesi ayrıca doğrulanmalıdır.</p></div></div></section>
