...
const ParamsSchema = z.object({ parcel_ids: z.array(z.string().uuid()).min(1).max(100) });
...
export const POST = async ({ request }: { request: Request }): Promise<Response> => {
  try {
    ...
    // The RPC performs the complete batch inside one database transaction.
    // If any parcel cannot be reserved, every reservation is rolled back.
    const { data: orders, error: orderError } = await supabase.rpc('create_parcel_orders_bulk', {
      p_parcel_ids: [...new Set(parse.data.parcel_ids)],
    });
    ...
    return jsonResponse({ ok: true, status: 'reserved', count: Array.isArray(orders) ? orders.length : 0, orders }, 202);
  } catch (error) {
